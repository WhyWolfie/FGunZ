...
ZScreenEffectManager::ZScreenEffectManager()
{
	...
	m_pScorePanel = NULL;
	m_pScoreNewPanel = NULL;
	m_pScoreTDMPanel = NULL;
	m_pScoreInfectedPanel = NULL;
	m_pBuffPanel = NULL;
...
void ZScreenEffectManager::Destroy()
{
	...
	SAFE_DELETE(m_pScoreNewPanel);
	SAFE_DELETE(m_pScoreTDMPanel);
	SAFE_DELETE(m_pScoreInfectedPanel);
...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	m_pScoreNewPanel = new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_tab_new.elu"));
	m_pScoreTDMPanel = new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_tab_tdm.elu"));
	m_pScoreInfectedPanel = new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_tab_infected.elu"));
...
void ZScreenEffectManager::DrawNewScoreBoard(bool bIsSolo)
{
	if (bIsSolo)
		m_pScoreNewPanel->Draw(0);
	else
	{
		if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_INFECTED)
			m_pScoreInfectedPanel->Draw(0);
		else
			m_pScoreTDMPanel->Draw(0);
	}
}

...
public:
	...
	void Create(MUID& uidItem, unsigned long int nItemID, int nCount=1, 
				bool bIsRentItem=false, int nRentMinutePeriodRemainder=RENT_MINUTE_PERIOD_UNLIMITED, int iMaxUseHour = 0, int nRarity = 0, int niLevel = 0)	// Update sgk 0614 [int iMaxUseHour = 0] 추가
	{
		m_UID = uidItem;
		m_nItemID = nItemID;

...
public:
	...
	// Custom: Changed return value (optimize no lead)
	...
	ZShotInfo *OnPeerShotgun_Damaged(ZObject* pOwner, float fShotTime, const rvector& pos, rvector &dir, ZPICKINFO pickinfo, DWORD dwPickPassFlag, rvector& v1, rvector& v2, ZItem *pItem, rvector& BulletMarkNormal, bool& bBulletMark, ZTargetType& nTargetType, bool& bHitEnemy);
	...
	void OnPeerAntiLead( const MUID& uidSender, MMatchCharItemParts nSelType, void* pZAntiLeadTemporaryInfoBlob );
	void OnVampire( const MUID& uidVictim, const MUID& uidAttacker, float fDamage );
	void OnPeerScope( const MUID& uidPeer, bool bScope );
	void OnKillstreak( const MUID& uidStreaker, const MUID& uidEnder, int nStreakType, int nKills );

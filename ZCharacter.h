...
//Custom: Removing ZAntiLeadTemporaryInfo
...
//	virtual void OnDamage(int damage, float fRatio = 1.0f);
	...
	// Custom: Disabled anti-lead for infected
	//virtual void OnDamagedAPlayer(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio=1.f, int nMeleeType=-1);
	//virtual void OnDamagedAPlayer(ZObject* pAttacker, vector<MTD_ShotInfo*> vShots);

	//Custom: Antilead update: this can be re-enabled later if you want to trick people.
	...
	//Custom: New antilead
	void OnDamaged_AntiLead(ZObject* pOwner, vector<ZShotInfo*> vInfo);

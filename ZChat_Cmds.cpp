...
void ChatCmd_CW(const char* line,const int argc, char **const argv);
void ChatCmd_Credits(const char* line,const int argc, char **const argv);
void ChatCmd_Freestyle(const char* line,const int argc, char **const argv);
void ChatCmd_LadderRejoin(const char* line,const int argc, char **const argv);
void ChatCmd_RemovePassword(const char* line, const int argc, char **const argv);
...
{	
	...
	_CC_AC("cw",						&ChatCmd_CW,				CCF_ALL, ARGVNoMin, 1, true, "/cw <enable/disable/admins/status>", "Sets the clan war option");
	_CC_AC("credits",					&ChatCmd_Credits,			CCF_ALL, ARGVNoMin, ARGVNoMax, true, "/credits", "Whos y0 daddy");
	_CC_AC("freestyle",					&ChatCmd_Freestyle,			CCF_ALL, ARGVNoMin, ARGVNoMax, true, "/freestyle", "View our features");
	_CC_AC("rejoin",					&ChatCmd_LadderRejoin,		CCF_LOBBY, ARGVNoMin, ARGVNoMax, true, "/rejoin", "Rejoin clan war match");

	_CC_AC("removepassword",			&ChatCmd_RemovePassword,CCF_ADMIN|CCF_STAGE|CCF_GAME, ARGVNoMin, ARGVNoMax, true,"/removepassword", "");
	...
	_CC_AC("sbtest",					&ChatCmd_AdminSBTest,	CCF_ADMIN, ARGVNoMin, 1 , true,"/sbtest <charname>", "");
	...
	_CC_AC("admin_nat",					&ChatCmd_AdminNAT,		CCF_ADMIN, ARGVNoMin, ARGVNoMax, true, "/admin_nat", "Toggle NAT");
...
void ChatCmd_CW(const char* line, const int argc, char **const argv)
{
	...
	if( stricmp( argv[1], "enable" ) == 0 )
		...
		ZPostCWOption( 1 );
	else if( stricmp( argv[1], "admins" ) == 0 )
		ZPostCWOption( 2 );
...
void ChatCmd_Credits(const char* line, const int argc, char **const argv)
{
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_LOBBY_DEFALT), "Freestyle GunZ V7 (1.5)" );
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_BROADCAST), "- Coded by: Linear88, Mike, and SecretsOTheP." );
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_BROADCAST), "- Managed by: McSic, SuperWaffle and the developers" );
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_BROADCAST), "- Visit http://fgunz.net for more info!" );
}
...
void ChatCmd_Freestyle(const char* line, const int argc, char **const argv)
{
	...
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_BROADCAST), "- Coded by: Linear88, Mike, and SecretsOTheP." );
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_BROADCAST), "- Managed by: McSic, SuperWaffle and the developers" );
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_BROADCAST), "-- Type /commands for a list of our game commands" );
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_BROADCAST), "-- Type /mods for a list of our room modifiers" );
	ZChatOutput( MCOLOR(ZCOLOR_CHAT_BROADCAST), "-- Music Player: Put your mp3s in CUSTOM/music!" );
}

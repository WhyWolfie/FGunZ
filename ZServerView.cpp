	...
	for ( int nRowBlock = 0;  nRowBlock < 2/*nRow*/;  nRowBlock++)
	{
		for ( int nColBlock = 0;  nColBlock < 3/*nColBlock*/;  nColBlock++)
		{
			...
			for ( SERVERLIST::iterator itr = m_cServerList.begin();  itr != m_cServerList.end();  itr++)
			{
				...
				// Custom: Made NHNUSA locale use Netmarble's stylish server box
#if defined(LOCALE_KOREA) || defined(_DEBUG) || defined(LOCALE_NHNUSA)
...
bool ZServerView::AddServer( char* szName, char* szAddress, int nPort, int nType, int nNumOfUser, int nCapacity, bool IsLive, char* szAgentIP, int nRow, int nCol, int nPeerPort)
{
	...
	pServerNode->nPeerPort = nPeerPort;

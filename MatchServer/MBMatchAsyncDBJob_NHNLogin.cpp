...
void MBMatchAsyncDBJob_NHNLogin::Run( void* pContext )
{
...
#ifdef LOCALE_NHN_USA_DIS // Custom: Disabled NHN auth
	...
	TCHAR szSalt[ 64 ] = {0,}; // Custom: Salt
	if( !pDBMgr->GetLoginInfo(m_strUserID.c_str(), &m_nAID, szDBPassword, szSalt) )
	{
		// 처음 접속하는 유저.
		...
		pDBMgr->GetLoginInfo(m_strUserID.c_str(), &m_nAID, szDBPassword, szSalt);
	}

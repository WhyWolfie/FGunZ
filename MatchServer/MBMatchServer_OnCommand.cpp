...
// #endif
	...
	case MC_MATCH_REQUEST_SELL_ITEM:
		...
		break;
	//Custom case: Request Sacrifice
	case MC_MATCH_REQUEST_SAC_ITEM:
		{
			MUID uidPlayer, uidItem;
			int nItemCount = 0;
			uidPlayer = pCommand->GetSenderUID();			
			...
			OnRequestSacItem(uidPlayer, uidItem, nItemCount);
		}
		break;
	//Custom: Logs DLLs loaded in the client on the server. Zlib compressed.
	case MC_DEBUGINFO:
		{
			MUID uidPlayer;
			uidPlayer = pCommand->GetSenderUID();
			...
			if(!pPlayer || pPlayer && !IsEnabledObject(pPlayer)) 
				break;

			if(!pPlayer->GetAccountInfo())
				break;

			MCommandParameter* pParam = pCommand->GetParameter(0);
			if(pParam == NULL || pParam->GetType()!=MPT_BLOB) break;

			void* pBlob = pParam->GetPointer();
			if(pBlob == NULL) break;

			if(MGetBlobArrayCount(pBlob) != 1) break;
			int nSize = MGetOneBlobSize(pBlob);
			if(nSize > 4096)	break;

			BYTE* szModuleData = (BYTE*)MGetBlobArrayElement(pBlob, 0);
			if(!szModuleData)	break;

			//for debug
			BYTE* uncompresseddata = new BYTE[12000];
			DWORD uncompressedsize = 12000;
			uncompress(uncompresseddata, &uncompressedsize, (BYTE*)szModuleData, nSize);

			//LogInfo(peerId, "SaveModule", "uncomp %i, comp %i", uncompressedsize, size);

			std::string uncompressedstr((char*)uncompresseddata, uncompressedsize);

			std::stringstream os(std::stringstream::in | std::stringstream::out | std::stringstream::binary);
			uncompressedstr.resize(uncompressedsize);
			os.str(uncompressedstr);
			unsigned int dataToRead = 0;
			std::vector<std::pair<std::string, std::string>> module_list;
			while(!os.eof())
			{
				if(sizeof(unsigned int) > os.str().length() || (os.fail()))
					break;
				os.read((char*)&dataToRead, sizeof(unsigned int));
				//LogInfo(peerId, "SaveModule", "len %i", dataToRead);

				if(dataToRead > os.str().length() || (os.fail()))
				{
					//	LogInfo(peerId, "SaveModule", "shit broke: %i %i", data, os.str().length());
					break;
				}

				char* bufToRead = new char[MAX_PATH];
				os.read(bufToRead, dataToRead);
				bufToRead[dataToRead] = '\0';
				//LogInfo(peerId, "SaveModule", "push it real good: %s:%i", bufToRead, os.str().length());
				if(sizeof(unsigned int) > os.str().length() || (os.fail()))
					break;
				...
				if(dataToRead > os.str().length() || (os.fail()))
					break;
				char* bufToRead2 = new char[MAX_PATH];
				os.read(bufToRead2, dataToRead);
				bufToRead2[dataToRead] = '\0';
				//LogInfo(peerId, "SaveModule", "push it real good 2: %s", bufToRead2);
				module_list.push_back(std::make_pair(bufToRead, bufToRead2));
				SAFE_DELETE_ARRAY(bufToRead);
				SAFE_DELETE_ARRAY(bufToRead2);
			}


			std::vector<std::pair<std::string, std::string>>::iterator it = module_list.begin();
		
			while(it!=module_list.end())
			{
				MBMatchAsyncDBJob_LogCharModules* pJob = new MBMatchAsyncDBJob_LogCharModules(uidPlayer);
				if( 0 == pJob ) 
					break;
				...
				pJob->Input(pPlayer->GetAccountInfo()->m_nAID, (*it).first.c_str(), (*it).second.c_str());
			...
			module_list.clear();
			os.clear();
			SAFE_DELETE_ARRAY(uncompresseddata);
		}
		break;

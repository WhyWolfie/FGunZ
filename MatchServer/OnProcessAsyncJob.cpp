...
void MBMatchServer::OnAsyncBringAccountItem(MAsyncJob* pJobResult)
{
	...
	//Custom: iLevel/Rarity for Quest
	int nRarity							= pJob->GetOutRarity();
	int niLevel							= pJob->GetOutiLevel();
	...
	pObj->GetCharInfo()->m_ItemList.CreateItem(MMatchItemMap::UseUID(), noutCIID, noutItemID, 
		boutIsRentItem, noutRentMinutePeriodRemainder, woutRentHourPeriod, 1, nRarity, niLevel);
...
void MBMatchServer::OnAsyncBringAccountItemStackable(MAsyncJob* pJobResult)
{
	...
	if( pItemDesc )
	{
		...
		{
			pObj->GetCharInfo()->m_ItemList.CreateItem(MMatchItemMap::UseUID(), noutCIID, noutItemID, 
				boutIsRentItem, noutRentMinutePeriodRemainder, woutRentHourPeriod, noutItemCnt, niLevel, nRarity);
		} 
		else 
		{

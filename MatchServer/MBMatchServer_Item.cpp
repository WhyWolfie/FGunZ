...
bool MBMatchServer::ResponseCharacterItemList(const MUID& uidPlayer)
{
	...
	for (MMatchItemMap::iterator itor = pItemList->begin(); itor != pItemList->end(); ++itor) {
		...
		//Custom: load pItem Rarity/iLevel
		niLevel					= pItem->GetiLevel();
		nRarity					= pItem->GetRarity();
...
//Custom: OnRequest handler for Sacrificing items
void MBMatchServer::OnRequestSacItem(const MUID& uidPlayer, const MUID& uidItem, int nItemCount)
{

	//Disabled for the time being. Content isn't done yet :X
	MMatchObject* pObj = GetObject( uidPlayer );
	if( NULL == pObj ) return;
	...
	if( NULL != pCharInfo->m_ItemList.GetItem(uidItem) ) 
	{
		...
		if( pItem == NULL ) return;
		if( pItem->GetDesc() == NULL ) return;
		if( pItem->GetDesc()->IsSpendableItem()) return; //Don't sacrifice spendable items.
		if( pItem->GetDesc()->IsCashItem()) return; //Don't sacrifice cash items. :X
		ResponseSacItem(uidPlayer, uidItem);			
	}
...
//Custom: Not used. Yet.
bool MBMatchServer::ResponseSacItem(const MUID& uidPlayer, const MUID& uidItem)
{
	MMatchObject* pObj = GetObject( uidPlayer );
	if( pObj == NULL ) return false;
	...
	if(NULL == pStage) return false;

	if(!pStage->GetStageSetting())	return false;
	if(pStage->GetStageSetting()->GetGameType() != MMATCH_GAMETYPE_QUEST)
		return false;

	MMatchItem* pItem = pObj->GetCharInfo()->m_ItemList.GetItem(uidItem);
	if( pItem == NULL || pItem->GetDesc() == NULL ) return false;
	MMatchRuleQuest* pRuleQuest = reinterpret_cast<MMatchRuleQuest*>(pStage->GetRule());
	if(!pRuleQuest) return false;

	// Removed because of McSic's request
	//if(uidPlayer != pStage->GetMasterUID())
	//	return false;
	
	/*Disabled. Re-enable for item cap:
	if(pItem->GetiLevel() > 15)
	{
		char szMsg[1024];
		MCommand* pCmd = MMatchServer::GetInstance()->CreateCommand(MC_MATCH_ANNOUNCE, MUID(0,0));
		pCmd->AddParameter(new MCmdParamUInt(0));
		sprintf(szMsg, "%s%d", "Sacrificing items at the level of item requested is disabled for the moment. Your item has not been sacrificed. Level:  ", pItem->GetiLevel());
		pCmd->AddParameter(new MCmdParamStr( szMsg));
		MMatchServer::GetInstance()->RouteToStage(pStage->GetUID(), pCmd);
		return false;
	}*/

	if(pRuleQuest->GetScaled()) return false;

	pRuleQuest->SetScaling(pItem->GetiLevel());

	/////////////////////////////////////////////////////////////////////////////////////////////
	UpdateCharDBCachingData(pObj);		///< XP, BP, KillCount, DeathCount updates
	/////////////////////////////////////////////////////////////////////////////////////////////
	...
	MCommand* pCmd = MMatchServer::GetInstance()->CreateCommand(MC_MATCH_ANNOUNCE, MUID(0,0));
	pCmd->AddParameter(new MCmdParamUInt(0));
	sprintf(szMsg, "%s%d", "The difficulty of the quest has been increased to level: ", pItem->GetiLevel());
	pCmd->AddParameter(new MCmdParamStr( szMsg));
	MMatchServer::GetInstance()->RouteToStage(pStage->GetUID(), pCmd);
	return true;
}

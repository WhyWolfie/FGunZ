...
void MBMatchServer::ProcessOnGameKill(MMatchStage* pStage, MMatchObject* pAttacker, MMatchObject* pVictim)
{
	...
	*/
	...
	int nAttackerKillstreak = pAttacker->GetKillstreak();
	int nVictimKillstreak = pVictim->GetKillstreak();

	// Custom: Handle attacker killstreaks
	if( nAttackerKillstreak > 0 && nAttackerKillstreak <= 10 )
	{
		int nKillstreak = pAttacker->GetKillstreak() % 10;
		int nStreakType = 0;

		// nStreakType -> 5 is reserved for killstreak end
		switch (nKillstreak)
		{
			case 4:
				nStreakType = 1;
		...
		if( nStreakType != 0 )
		{
			MCommand* pCmd = CreateCommand(MC_MATCH_KILLSTREAK, MUID(0,0));
			pCmd->AddParameter(new MCommandParameterUID(pAttacker->GetUID()));
			pCmd->AddParameter(new MCommandParameterUID(MUID(0,0)));
			pCmd->AddParameter(new MCommandParameterInt(nStreakType));
			pCmd->AddParameter(new MCommandParameterInt(nAttackerKillstreak));
			RouteToBattle(uidStage, pCmd);
		}
	}

	// Handle victim killstreak ends
	// End any possible killstreak that's building up
	pVictim->SetKillstreak(0);

	// If a killstreak message was already in place, end it.
	// Note that nVictimKillstreak is a cached value on stack.
	if( nVictimKillstreak >= 4 )
	{
		MCommand* pCmd = CreateCommand(MC_MATCH_KILLSTREAK, MUID(0,0));
		...
		if( bSuicide )
			...
			pCmd->AddParameter(new MCommandParameterUID(pAttacker->GetUID()));
		pCmd->AddParameter(new MCommandParameterInt(5)); // End killstreak
		pCmd->AddParameter(new MCommandParameterInt(nVictimKillstreak));
		RouteToBattle(uidStage, pCmd);
	}
...
int MBMatchServer::ValidateStageJoin(const MUID& uidPlayer, const MUID& uidStage)
{
	...
	if (!IsAdminGrade(pObj))
	{
		...
		if (pObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM)
		{
			// Allow full room joining for ALL rooms
			//MMatchChannel* pChannel = FindChannel( pStage->GetOwnerChannel() );
			//if (pChannel && pChannel->IsUseEvent())
			return MOK;
		}
		...
		{
			// Custom: VIP/Premium members AND Event Team can access full rooms
			if (pObj->GetAccountInfo()->m_nPGrade != MMPG_PREMIUM_IP || (pObj->GetAccountInfo()->m_nPGrade == MMPG_PREMIUM_IP && pStage->GetVIPObjCount() >= 2))

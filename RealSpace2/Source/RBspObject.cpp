...
void RBspObject::SetDiffuseMap(int nMaterial)
{
	...
	if(pTex)
	{
		...
		if(GetWhiteMap() == 1) pd3dDevice->SetTextureStageState( 0, D3DTSS_COLORARG1, 0xFFFFFFFF ); //Custom: No need to replace textures lel
		else pd3dDevice->SetTextureStageState( 0, D3DTSS_COLORARG1, D3DTA_TEXTURE );
	}
	else
	{

...
//Custom: Sacrifice button on Shop.
void ZShopEquipInterface::OnShopSacButton(void)
{
	static unsigned long int st_LastRequestTime = 0;	///< 막 누르는 것 방지
	unsigned long int nNowTime = timeGetTime();
	if ((nNowTime - st_LastRequestTime) < 1000) return;
	...
	if (!pListItem->GetItemData()->GetHandleSac()) return;

	if (pListItem->GetItemData()->GetHandleSac())
		pListItem->GetItemData()->GetHandleSac()->Sell();
}

void ZShopEquipInterface::OnEquipSacButton(void)
{
	static unsigned long int st_LastRequestTime = 0;	///< 막 누르는 것 방지
	unsigned long int nNowTime = timeGetTime();
	if ((nNowTime - st_LastRequestTime) < 1000) return;
	...
	ZShopEquipListItem* pListItem = (ZShopEquipListItem*)GetListCurSelItem("EquipmentList");
	if (!pListItem) {
		return;
	}
...
MMatchCharItemParts ZShopEquipInterface::RecommendEquipParts(MMatchItemSlotType slot)
{
	...
	else if (slot == MMIST_HEAD)
	{
		if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_HEAD) == 0)
		{
			parts = MMCIP_HEAD;
		}
		else if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_AVATAR_HEAD) == 0)
		{
			parts = MMCIP_AVATAR_HEAD;
		}
		else
		{
			parts = MMCIP_HEAD;
	...
	else if (slot == MMIST_HANDS)
	{
		if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_HANDS) == 0)
		{
			parts = MMCIP_HANDS;
		}
		else if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_AVATAR_HANDS) == 0)
		{
			parts = MMCIP_AVATAR_HANDS;
		}
		else
		{
			parts = MMCIP_HANDS;
	...
	else if (slot == MMIST_CHEST)
	{
		if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_CHEST) == 0)
		{
			parts = MMCIP_CHEST;
		}
		else if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_AVATAR_CHEST) == 0)
		{
			parts = MMCIP_AVATAR_CHEST;
		}
		else
		{
			parts = MMCIP_CHEST;
	...
	else if (slot == MMIST_LEGS)
	{
		if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_LEGS) == 0)
		{
			parts = MMCIP_LEGS;
		}
		else if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_AVATAR_LEGS) == 0)
		{
			parts = MMCIP_AVATAR_LEGS;
		}
		else
		{
			parts = MMCIP_LEGS;
	...
	else if (slot == MMIST_FEET)
	{
		if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_FEET) == 0)
		{
			parts = MMCIP_FEET;
		}
		else if (ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMCIP_AVATAR_FEET) == 0)
		{
			parts = MMCIP_AVATAR_FEET;
		}
		else
		{
			parts = MMCIP_FEET;
...
bool ZShopEquipInterface::Equip(void)
{
	...
	WidgetHideDisableShow("SacEquipConfirmCaller");
...
bool ZShopEquipInterface::Equip(MMatchCharItemParts parts, MUID& uidItem)
{
	...
	if(!ZGetGameClient()->GetMatchStageSetting()->IsUserExempt(ZGetMyInfo()->GetUGradeID())) // Custom: Staff Exception Check
	{
			...
			ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_NI ))
		{
			...
			if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_NG ) && (parts == MMCIP_PRIMARY || parts == MMCIP_SECONDARY) )
				return false;

			if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_NI ) && (parts == MMCIP_CUSTOM1 || parts == MMCIP_CUSTOM2) )
				return false;
		}
	}
...
{
	//Custom: Avatar Clothing Logic for 'similar' clothing slot types.
	switch (nSlotType)
	{
	case MMIST_MELEE :
	...
	case MMIST_CUSTOM :
		...
		break;
	case MMIST_HEAD :
		if ( nParts == MMCIP_HEAD || nParts == MMCIP_AVATAR_HEAD)
			return true;
		break;
	case MMIST_CHEST :
		if ( nParts == MMCIP_CHEST || nParts == MMCIP_AVATAR_CHEST)
			return true;
		break;
	case MMIST_HANDS :
		if ( nParts == MMCIP_HANDS || nParts == MMCIP_AVATAR_HANDS)
			return true;
		break;
	case MMIST_LEGS :
		if ( nParts == MMCIP_LEGS || nParts == MMCIP_AVATAR_LEGS)
			return true;
		break;
	case MMIST_FEET :
		if ( nParts == MMCIP_FEET || nParts == MMCIP_AVATAR_FEET)
			return true;
		break;
	case MMIST_FINGER :
...
void ZShopEquipInterface::SelectShopTab(int nTabIndex)
{
	...
	//Custom: Sacrifice Button
	pButton = (MButton*)pResource->FindWidget( "SacShopConfirmCaller");
	if ( pButton)
	{
		pButton->Show( false);
		pButton->Enable( false);
	...
	else if ( nTabIndex == 1)
	{
		...
		if ( pButton)
			pButton->Show( true);
		//Custom: Sacrifice Button enable conditions
		if(ZGetGameClient() && ZGetGameClient()->GetMatchStageSetting() && ZGetGameClient()->GetMatchStageSetting()->GetStageSetting())
		{
			const MSTAGE_SETTING_NODE* pStageSetting = ZGetGameClient()->GetMatchStageSetting()->GetStageSetting();
			if(pStageSetting->nGameType == MMATCH_GAMETYPE_QUEST)
			{
...
void ZShopEquipInterface::SelectEquipmentTab(int nTabIndex)
{
	...
	//Custom: Sacrifice Button enable conditions
	pButton = (MButton*)pResource->FindWidget( "SacEquipConfirmCaller");
	if ( pButton)
	{
		pButton->Show( false);
		pButton->Enable( false);

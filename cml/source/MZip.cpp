...
{
	// Custom: (Note) This function is only used to decrypt Mrsv1 / Zip files!
	if(!pData) return;
...
void RecoveryChar(char* pData,int _size)
{
	...
	// Custom: ZIP IV for static encryption
	static char zipIV[] = { 0x5, 0x64, 0x33, 0x99, 0x77, 0x23, 0x11, 0x59, 0x91, 0x11, 0x26, 0x21, 0x88, 0x84, 0x75, 0x28, 0x33, 0x15 };

	// Custom: FGZ decryption
	for( int i = 0; i < _size; ++i )
	{
		pData[i] ^= zipIV[i % 18];
	}
	
	// Custom: Default MRS decryption
	/*
	for( int i = 0; i < _size; ++i )
	{
		BYTE a = pData[i];
		pData[i] = (byte)(~((a >> 3) | (a << 5)));
	}
	*/
...
void RecoveryCharData(char* pData, int _size)
{
	// Custom: ZIP Data IV for static encryption
	static char zipDataIV[] = { 0x22, 0x1, 0x57, 0x43, 0x24, 0x68, 0x87, 0x22 };

	for( int i = 0; i < _size; ++i )
	{
		pData[i] ^= zipDataIV[i % 8];
	}
}
...
bool MZip::Initialize(FILE* fp,unsigned long ReadMode)
{
	...
	// Decrypt/Recover the EOCD for MRS v2
	...
	{
		if( m_nZipMode>=ZMode_Mrs2 )
			RecoveryChar( (char*)pfh , 46 ); // before name
		...
		// MZIPDIRFILEHEADER (EOCD File Header)
		pfh += sizeof(MZIPDIRFILEHEADER);

		if( m_nZipMode>=ZMode_Mrs2 )
			RecoveryChar( (char*)pfh , fh.fnameLen );
...
bool MZip::ReadFile(int i, void* pBuffer, int nMaxSize)
{
	...
	if(h.compression==MZIPLOCALHEADER::COMP_STORE){
		...
		RecoveryCharData((char*)pBuffer, h.cSize); // FGZ
		return true;
	}
	...
	RecoveryCharData((char*)pData, h.cSize); // FGZ

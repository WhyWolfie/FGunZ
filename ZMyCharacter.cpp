...
void ZMyCharacter::OnShotRange()
{
	...
	if ((pSelectedItem->GetDesc()->m_nWeaponType.Ref() == MWT_SNIFER) && (zStatus.m_bSniferMode))
	{
		...
		if( pCamera )
		{
			...
			pCamera->m_fAngleX -= 0.15f; // Throw camera up
...
void ZMyCharacter::OnGadget_Snifer()
{
	...
	if( GetTickCount() - g_dwLastScopeTime < 1100 )
		return;
...
#define GUARD_TIME	0.1f
	...
	if(bBothPressed)
	{
		...
		{
			if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_NOKSTYLE ) )
			{
...
//	if(m_bBlastDrop && GetDistToFloor()<5.f && GetVelocity().z<1 && !m_bDrop)
	...
	// Custom: Shield mod
	if( (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_SHIELD ) && !IsDie()) || (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_ROLLTHEDICE ) && ZGetGame()->GetRolledDice() == 5) )
	{
...
void ZMyCharacter::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	// Custom: Shield
	if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_SHIELD ) || (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_ROLLTHEDICE ) && ZGetGame()->GetRolledDice() == 5) )
	{
		...
		&& ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_DUELTOURNAMENT)
	{
		if(pAttacker && !pAttacker->IsNPC())
		{
			...
			if(pCharacter && ZGetGame()->m_pMyCharacter && pCharacter != ZGetGame()->m_pMyCharacter)
			{

...
protected:
	...
	virtual void RouteGameInfoToSingle(MUID& uidPlayer) = 0;						///< 클라이언트에 게임 정보 보내준다.
	...
	virtual void RouteReadyToNewSector(const MUID& uidPlayer) = 0;
	virtual void RouteMovetoPortal(const MUID& uidPlayer) = 0;
	void ReAssignNPC();
...
public:
	...
	virtual void OnQuestEnterBattle(MUID& uidChar);			///< 게임중 난입할때 호출된다.
	virtual void PostNPCInfoForSingle(MUID& uidPlayer) {}
	virtual void OnQuestStageLaunch();			///< 게임중 난입할때 호출된다.

...
protected:
	...
	void OnMatchLogin(MUID CommUID, const char* szUserID, const char* szPassword, int nCommandVersion, unsigned long nChecksumPack, char *szEncryptMd5Value, char* szEncryptGunz64Md5Value, char *szEncryptSystemMd5Value, char *szEncryptSecretValue, char *szEncryptAHMd5Value, char* szHwid);
...
protected:
	...
	void OnClanSetCWOption( const MUID& uidComm, int nOption );
	void OnRequestRoll(const MUID& uidRoller);
	void OnEventRemovePassword(const MUID& uidAdmin);
...
public:
	...
	bool DistributeZItem(const MUID& uidPlayer, const unsigned long int nItemID, bool bRentItem, int nRentPeriodHour, int nItemCount, int nRarity, int niLevel);

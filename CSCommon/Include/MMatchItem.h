...
public:
	...
	bool Create( const MUID& uid, MMatchItemDesc* pDesc, const WORD nCountOfNonDesctroyItem, int nCount = 1, int Rarity = 0, int iLevel = 0);
	void Destroy();
	...
	int	GetRarity() const					{ return m_nRarity; }
	int GetiLevel() const					{ return m_niLevel; }
	void SetRarity(int Rarity) 					{ m_nRarity = Rarity; }
	void SetiLevel(int iLevel)					{  m_niLevel = iLevel; }

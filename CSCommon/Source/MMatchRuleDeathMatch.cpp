...
void MMatchRuleGunGame::OnBegin()
{
	...
	if (pStage != NULL)
	{
		...
		{
			MGunGameData gungameData = GetGunGameData();
			vector<int> checked;

			random_shuffle(gungameData.begin(), gungameData.end());
			for (int i = 0; i < gungameData.size(); ++i)
			{
				if (find(checked.begin(), checked.end(), gungameData[i].nLevel) == checked.end())
				{
					checked.push_back(gungameData[i].nLevel);
				}
			}
			...
			for (int i = 0; i < checked.size(); ++i)
			{
				for (int j = 0; j < gungameData.size(); ++j)
				{
					...
					{
						GunGameData cpy;
						cpy.nItemID = gungameData[j].nItemID;
						cpy.nItemParts = gungameData[j].nItemParts;
						cpy.nLevel = i + 1;

						temp.push_back(cpy);
						//g_GunGameData[j].nLevel = i + 1;
					}
				}
			}

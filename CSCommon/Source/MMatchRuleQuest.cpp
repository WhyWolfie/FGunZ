	...
	case MQUEST_COMBAT_PLAY:
		{
			...
			{
				MQuestPlayerInfo* pPlayerInfo = (*itor).second;
				//Custom: Assigns the 'restricted exp by iLevel' to a quest based on the highest iLevel in the room.
				if(pPlayerInfo->pObject && pPlayerInfo->pObject->GetCharInfo())
				{
					MMatchCharInfo* pInfo = pPlayerInfo->pObject->GetCharInfo();
					for (int i = 0; i < MMCIP_END; i++) 
					{
						if (!pInfo->m_EquipedItem.IsEmpty(MMatchCharItemParts(i))) 
						{
							MMatchItem* pItem = pInfo->m_EquipedItem.GetItem((MMatchCharItemParts)i);
							if(pItem && pItem->GetiLevel() > m_StageGameInfo.nHighestItemPerRound)
							{
								this->m_StageGameInfo.nHighestItemPerRound = pItem->GetiLevel();
							}
						}
					}
				}
...
void MMatchRuleQuest::OnSectorCompleted()
{
	...
	if (pScenario)
	{
		...
		//Custom: Must be +/- 15 iLevel within quest to gain exp
...
{
	//Must be within +/- 15 iLevels for exp in quest modes.
	signed int nTotal =  (signed int)( (signed int)(m_StageGameInfo.nHighestItemPerRound + (signed int)100) - (signed int)m_StageGameInfo.nScaling);
...
bool MMatchRuleQuest::DistributeZItem( MQuestPlayerInfo* pPlayerInfo, void** ppoutQuestRewardZItemBlob)
{
	...
	for(MQuestRewardZItemList::iterator itor = pObtainZItemList->begin(); itor != pObtainZItemList->end(); ++itor )
	{
		...
		//Custom: Rarity/iLevel drop chances.

		//Custom: These following variables determine what iLevels drop, and what the percentage chance at rarities are.
		//nRarityBase is for the iLevel range. We subtract 100 to get the iLevel without the scaling percent.
		int nRarityBase = m_StageGameInfo.nScaling - 100;
		
		//nRarityRangeLow is the minimum iLevel that can drop out of a quest and,
		int nRarityRangeLow = 0;
		if(nRarityBase >= 5)
			nRarityRangeLow = nRarityBase - 5;
		//nRarityRangeHigh is the max iLevel that can drop out of a quest.
		int nRarityRangeHigh = nRarityBase + 5;
	
		//Make a random number between min and max for this item.
		int niLevelRoll = RandomNumber(nRarityRangeLow, nRarityRangeHigh);

		//This 'roll' is for Rarity itself.
		int roll = RandomNumber(0, 10000);
		
		//A chance to get common items by default is about the same as uncommon/rare items.
		int nRarity = 0;

		if(roll >= 2500) // 3/4th chance to get uncommon
		{
			nRarity = 1;
		}

		if (roll >= 5000) // 1/2 chance to get rare
		{
			nRarity = 2;
		}
		//However, if you get lucky, you can get Epic/Legendary...
		if (roll >= 9500) // 500 out of 10000 chance to get epic
		{
			nRarity = 3;
		}

		if(roll >= 9990) //10 out of 10000 chance to get legendary
		{
			nRarity = 4;
		}
		//Custom: Distribute Quest items: the item and its iLevel/Rarity
		MMatchServer::GetInstance()->DistributeZItem(pPlayer->GetUID(), iteminfo.nItemID, true, iteminfo.nRentPeriodHour, 1, niLevelRoll, nRarity);

								 ...
								 float fPointRatio)
{
	...
	else if (nOneTeamMemberCount == MLADDERTYPE_NORMAL_4VS4) // 4v4인전 포인트가 두배
...
void MMatchServer::OnClanSetCWOption( const MUID& uidComm, int nOption )
{
	MMatchObject* pObject = GetPlayerByCommUID(uidComm);
	if (! IsEnabledObject(pObject)) return;

	if (!pObject->GetCharInfo()->m_ClanInfo.IsJoined()) return;

	MMatchClan* pClan = m_ClanMap.GetClan(pObject->GetCharInfo()->m_ClanInfo.m_nClanID);
	if ((pClan == NULL) || (!pClan->IsInitedClanInfoEx())) return;
	...
	int nOldOption = pClan->GetCWOption();

	// uninitialized
	if (nOldOption == 255)
		pClan->InitClanInfoFromDB();

	nOldOption = pClan->GetCWOption() == 255 ? 0 : pClan->GetCWOption();

	if( nOption == 3 )
	{
		sprintf( szBuf, "[CW] Clan Wars are currently: " );

		if( nOldOption == 0 )
			strcat( szBuf, "Enabled (For Everyone)" );
		else if( nOldOption == 1 )
			strcat( szBuf, "Disabled (Except for master)" );
		else if( nOldOption == 2 )
			strcat( szBuf, "Master/Admins Only" );

		Announce( pObject, szBuf );
		return;
	}

	if( pObject->GetCharInfo()->m_ClanInfo.m_nGrade != MCG_MASTER )
	{
		Announce( pObject, "[CW] Only clan masters can use this feature." );
		return;
	}

	pClan->SetCWOption( nOption );
	if (!MMatchServer::GetInstance()->GetDBMgr()->UpdateCWOption( pClan->GetCLID(), nOption ))
	{
		Announce( pObject, "[CW] There was an error while processing your request." );
		return;
	}

	sprintf( szBuf, "[CW] Your old option was: " );
	...
	sprintf( szBuf, "[CW] New option set - " );

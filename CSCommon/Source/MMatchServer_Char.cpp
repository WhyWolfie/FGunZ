...
void MMatchServer::OnMatchBanMe(const MUID& uidComm, WarpHackType nType, void* pBlobArray)
{
	...
	else if (nType == WHT_MEMORY)
	{
		sprintf_s(szType, "Memory");
	}
	...
	{
		LOG(LOG_PROG, "Auto banned FAILED : AID(%u) Reason(%s)", pObj->GetAccountInfo()->m_nAID, szType );
	}
	else
	{
		LOG(LOG_PROG, "Auto banned : AID(%u) Reason(%s)", pObj->GetAccountInfo()->m_nAID, szType );

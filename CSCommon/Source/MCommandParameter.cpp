...
int MCommandParameterString::SetData(char* pData)
{
	...
	// Custom: Check string length. (Kore)
	if( nValueSize > 0 && nValueSize < 65533 )
	{
		if( isNotOKToReadMemory( pData, nValueSize ) )
			return 0;
...
int MCommandParameterBlob::SetData(char* pData)
{
	...
	// Custom: Check blob length. (Kore)
	if( m_nSize > 0 && m_nSize < 1048576 )
	{
		if( isNotOKToReadMemory( pData, m_nSize ) )
			return 0;

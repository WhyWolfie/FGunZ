...
MCommObject::MCommObject(MCommandCommunicator* pCommunicator)
{
	...
	m_pCommandBuilder = new MCommandBuilder(MUID(0,0), pCommunicator->GetUID(), pCommunicator->GetCommandManager());
}
...
void MCommandCommunicator::Run(void)
{
	...
	while(1)
	{
		...
		unsigned int CommandID = pCommand->GetID();
		...
		else if (pCommand->m_pCommandDesc->IsFlag(MCDT_LOCAL)==true || (m_This.IsValid() && pCommand->m_Receiver==m_This))
		{
			#ifdef _CMD_PROFILE
				m_CommandProfiler.OnRecv(pCommand);
				m_CommandProfiler.OnCommandBegin(pCommand, timeGetTime());

...
///////////////////////////////////////////////////////////////////////////////
// MMatchItem /////////////////////////////////////////////////////////////////
//Custom: iLevel Rarity Data Storage
...
bool MMatchItem::Create( const MUID& uid, MMatchItemDesc* pDesc, const WORD nCountOfNonDesctroyItem, int nCount, int Rarity, int iLevel)
{
	if( NULL == pDesc ) {
		m_pDesc = NULL;
		return false;
...
//Custom: iLevel Rarity Assignments
	m_niLevel					= iLevel;
	m_nRarity					= Rarity;
...
bool MMatchItemMap::CreateItem( const MUID& uid
							   ...
							   , int nCount, int Rarity, int iLevel)
{
	...
	//Custom: iLevel Rarity set functions on create for MMatchItem
	pNewItem->SetRarity(Rarity);
	pNewItem->SetiLevel(iLevel);
...
void MMatchEquipedItem::GetTotalWeight(int* poutWeight, int* poutMaxWeight)
{
	...
	{
		//Custom: Skip Avatar Clothing in calcs
...
bool IsSuitableItemSlot(MMatchItemSlotType nSlotType, MMatchCharItemParts nParts)
{
	...
	{
		if (nParts == MMCIP_HEAD || nParts == MMCIP_AVATAR_HEAD) return true;
		else return false;
	}
	else if (nSlotType == MMIST_CHEST)
	{
		if (nParts == MMCIP_CHEST || nParts == MMCIP_AVATAR_CHEST) return true;
		else return false;
	}
	else if (nSlotType == MMIST_HANDS)
	{
		if (nParts == MMCIP_HANDS || nParts == MMCIP_AVATAR_HANDS) return true;
		else return false;
	}
	else if (nSlotType == MMIST_LEGS)
	{
		if (nParts == MMCIP_LEGS || nParts == MMCIP_AVATAR_LEGS) return true;
		else return false;
	}
	else if (nSlotType == MMIST_FEET)
	{
		if (nParts == MMCIP_FEET || nParts == MMCIP_AVATAR_FEET) return true;
		else return false;
	}
...
MMatchCharItemParts GetSuitableItemParts(MMatchItemSlotType nSlotType)
{
	...
	case MMIST_HEAD:		return MMCIP_AVATAR_HEAD;
	case MMIST_CHEST:		return MMCIP_AVATAR_CHEST;
	case MMIST_HANDS:		return MMCIP_AVATAR_HANDS;
	case MMIST_LEGS:		return MMCIP_AVATAR_LEGS;
	case MMIST_FEET:		return MMCIP_AVATAR_FEET;

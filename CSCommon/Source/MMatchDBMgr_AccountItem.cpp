	...
	try {
		...
		for( ; ! rs.IsEOF(); rs.MoveNext() ) {
			...
			if ((bIsRentItem) && (nRentMinutePeriodRemainder < 0)) {
				...
				pOut[nodecount].nRarity						= rs.Field("Rarity").AsInt();
				pOut[nodecount].niLevel						= rs.Field("iLevel").AsInt();
...
bool MMatchDBMgr::BringAccountItem(const int nAID, const int nCID, const int nAIID, unsigned long int* poutCIID, 
								   unsigned long int* poutItemID, bool* poutIsRentItem, int* poutRentMinutePeriodRemainder, WORD& wRentHourPeriod, int* poutRarity, int* poutiLevel )
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	*poutItemID = rs.Field("ItemID").AsLong();	
	//Custom: iLevel Rarity Loading
	*poutRarity = rs.Field("Rarity").AsInt();
	*poutiLevel = rs.Field("iLevel").AsInt();
...
bool MMatchDBMgr::BringAccountItemStackable(const int nAID, const int nCID, const int nAIID, const int nCIID, const int nCount, unsigned long int* poutCIID, 
							   unsigned long int* poutItemID, bool* poutIsRentItem, int* poutRentMinutePeriodRemainder, unsigned int* poutCount, WORD& wRentHourPeriod, int* poutRarity, int* pOutiLevel )
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;

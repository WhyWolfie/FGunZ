...
void MMatchClan::InitClanInfoFromDB()
{
	...
	// Custom: Get CW Option
	int dbOption = 0;
	if( MMatchServer::GetInstance()->GetDBMgr()->GetCWOption(m_nCLID, &dbOption ) )
	{
		m_nCWOption = dbOption;
	}
	else
	{
		mlog("DB Query(GetCWOption) Failed\n");
	}
}

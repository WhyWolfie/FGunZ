...
bool MMatchServer::StageEnterBattle(const MUID& uidPlayer, const MUID& uidStage)
{
	...
	//	pNode->nPort = 0;
	//}
	if ((IsAdminGrade(pObj) || pObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM) && pObj->GetForcedNATOption() == true)
	{
...
void MMatchServer::OnPrivateStageJoin(const MUID& uidPlayer, const MUID& uidStage, char* pszPassword)
{
	...
	if (ugid == MMUG_EVENTMASTER || ugid == MMUG_DEVELOPER || ugid == MMUG_ADMIN) 
		bSkipPassword = true;

	// Custom: Event Team
	MMatchChannel* pOwnerChannel = FindChannel(pStage->GetOwnerChannel());
	if (pOwnerChannel && pOwnerChannel->IsUseEvent() && ugid == MMUG_EVENTTEAM)
		bSkipPassword = true;

	// 비밀방이 아니거나 패스워드가 틀리면 패스워드가 틀렸다고 응답한다.
	if(bSkipPassword==false) {
		// Custom: Added check for non-GMs joining ladder/DT matches
		if ((!pStage->IsPrivate()) || (strcmp(pStage->GetPassword(), pszPassword)) || (pStage->GetStageType() == MST_LADDER) || (pStage->GetStageSetting() && pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_DUELTOURNAMENT))
		{
			MMatchObject* pObj = GetObject(uidPlayer);
			if (pObj != NULL)
			{
...
void MMatchServer::OnVoteCallVote(const MUID& uidPlayer, const char* pszDiscuss, const char* pszArg)
{
	...
	// Custom: Event Team
	//MMatchChannel* pChannel = FindChannel(pObj->GetChannelUID());
	//if ((pObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM && pChannel && pChannel->IsUseEvent()) || IsAdminGrade(pObj)) {

	// Custom: Event Team global immunity
	...
	for (MUIDRefCache::iterator itor = pStage->GetObjBegin(); itor != pStage->GetObjEnd(); itor++) {
		...
		// Custom: Event Team
		//if ((pPlayer) && (IsAdminGrade(pPlayer) || (pPlayer->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM && pChannel && pChannel->IsUseEvent())))

		// Custom: Event Team global immunity
		if ((pPlayer) && (pPlayer->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM || IsAdminGrade(pPlayer)))
		{
...
// Custom: Remove password from stage
void MMatchServer::OnEventRemovePassword(const MUID& uidAdmin)
{
	MMatchObject* pObj = GetObject(uidAdmin);
	if( 0 == pObj ) 
		return;
...
void MMatchServer::OnEventRequestJjang(const MUID& uidAdmin, const char* pszTargetName)
{
	...
	MMatchObject* pTargetObj = GetPlayerByName(pszTargetName);
	if (pTargetObj == NULL) return;
	if (IsAdminGrade(pTargetObj) || pTargetObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM) return;		// 어드민 대상으로 짱불가
...
void MMatchServer::OnAdminPKick(const MUID& uidComm, char* pszTargetName, bool bBan)
{
	...
	if( bBan && pObj->GetStageUID() != pTargetObj->GetStageUID() )
		return;
	...
	{
		if( bBan )
			pStage->KickBanPlayer( pszTargetName, true );
		else
			pStage->UnbanKickedPlayer( pszTargetName );
	}

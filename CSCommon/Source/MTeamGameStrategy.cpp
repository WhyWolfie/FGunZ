...
{
	if(!LoadCWMapsFromXML()) //Custom: load defaults if no xml
	{
...
//Custom: Load CW Maps from an XML file by MapID and pair type.
bool MClanGameStrategy::LoadCWMapsFromXML()
{
	#define MTOK_MAP "MAP"
	...
	if (!xmlIniData.LoadFromFile("cwmaps.xml"))
	{
		xmlIniData.Destroy();
		return false;
	}
	...
	for (int i = 0; i < iCount; i++)
	{
		...
		if (!strcmp(szTagName, MTOK_MAP))
		{
			int	nMapID			= 0;
			int	nRuleSetType	= 0;
			int	nChance			= 0;
			childElement.GetAttribute(&nMapID, "mapid");
			childElement.GetAttribute(&nRuleSetType, "ruleset");
			childElement.GetAttribute(&nChance, "chance");

			if(nRuleSetType < MLADDERTYPE_MAX) //sanity check for compiler
			{
				InsertLadderRandomMap(m_RandomMapVec[nRuleSetType], nMapID, nChance);
			}
		}
	}

...
//			@GIID			INT
//			@GambleCount	INT  
//Custom: iLevel Rarity Addition
TCHAR g_szDB_INSERT_DISTRIBUTE_ITEM[] = _T("{CALL spInsertDistributeItem(%d, %d, %d, %d, %d)}");			
...
// Desc  : Get HWID and check if it is banned
// Param :	@HWID				VARCHAR(256)

TCHAR g_szDB_BAN_HWID[] = _T("{CALL spBanHWID(%d, '%s', %d)}");
// Desc  : Get HWID and check if it is banned
// Param :  @AID				INT
// Param :	@HWID				VARCHAR(256)
// Param :	@AutoReason			INT

TCHAR g_szDB_GET_USER_WARNINGS[] = _T("{CALL spGetUserWarnings(%d)}");
// Desc  : Get Warning Count
// Param :	@AID				INT

TCHAR g_szDB_GET_CLAN_CW_OPTION[] = _T("{CALL spGetClanCWOption(%d)}");
// Desc  : Get Clan Option
// Param :	@CLID				INT

TCHAR g_szDB_UPDATE_CLAN_CW_OPTION[] = _T("UPDATE Clan SET cwOption=%d WHERE CLID=%d");
// Desc  : Update Clan Option
// Param :	@cwOption			INT
// Param :	@CLID				INT
...
//Custom: iLevel Rarity Addition
	...
	for( ; ! rs.IsEOF(); rs.MoveNext() ) {
		...
		if( rs.Field("iLevel").IsNull() ) 	niLevel = 0;
		else							niLevel = rs.Field("iLevel").AsInt();

		if( rs.Field("Rarity").IsNull() ) 	Rarity = 0;
		else							Rarity = rs.Field("Rarity").AsInt();
...
//Custom: iLevel Rarity Addition
			pCharInfo->m_ItemList.CreateItem(MMatchItemMap::UseUID(), dwCIID, nItemDescID, bIsRentItem, nRentMinutePeriodRemainder, wMaxUsableHour, nCnt, Rarity, niLevel);
...
//Custom: Log character modules on entering a stage's battle state, leaving a stage's battle state, leaving a stage (ZGame::Destroy), or selecting a character.
bool MMatchDBMgr::LogCharModules(const int nAID, const char* nModule, const char* MMD5)
{
	if (!CheckOpen()) return false;

	CString strSQL;
	MCleanSQL( (char*)nModule ); //Custom: SQL injection fix as recommended by Dave prior to deployment.
	MCleanSQL( (char*)MMD5 ); //Custom: SQL injection for MD5s. Shouldn't be needed but prevention against malicious users never hurts...
	try {		
		strSQL.Format(g_szDB_UPDATE_CHAR_MODULES_LOG, nAID, nModule, MMD5);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) { //Custom: This function can gracefully fail, for rare, edge cases.
		return false;
	}
									   ...
									   unsigned long int* poutCIID)
{
	...
	CString strSQL;
	//Custom: iLevel Rarity Addition
	strSQL.Format(g_szDB_INSERT_DISTRIBUTE_ITEM, nCID, nItemDescID, nRentPeriodHour, nRarity, niLevel);
	CODBCRecordset rs(&m_DB);
...
bool MMatchDBMgr::GetCWOption( int nCLID, int* poutOption )
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	strSQL.Format(g_szDB_GET_CLAN_CW_OPTION, nCLID);
	...
	*poutOption = rs.Field("resultOption").AsInt();
	return true;
}

bool MMatchDBMgr::UpdateCWOption( int nCLID, int nOption )
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		strSQL.Format(g_szDB_UPDATE_CLAN_CW_OPTION, nOption, nCLID);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e)
	{

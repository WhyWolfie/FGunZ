...
// Custom: Added Hwid, Gunz64 MD5, System MD5, Secret String
void MMatchServer::OnMatchLogin(MUID CommUID, const char* szUserID, const char* szPassword, int nCommandVersion, unsigned long nChecksumPack, char *szEncryptMd5Value, char* szEncryptGunz64Md5Value, char *szEncryptSystemMd5Value, char *szEncryptSecretValue, char *szEncryptAHMd5Value, char* szHwid)
{
//	MCommObject* pCommObj = (MCommObject*)m_CommRefCache.GetRef(CommUID);
//	if (pCommObj == NULL) return;
	...
	unsigned int nAID = 0;
	char szDBPassword[128] = ""; // Custom: changed buffer len from 32
	...
	if (!m_MatchDBMgr.GetLoginInfo(szUserID, &nAID, szDBPassword, szSalt))
	{
		...
		m_MatchDBMgr.GetLoginInfo(szUserID, &nAID, szDBPassword, szSalt);
#endif
	...
	if( strlen(szSalt) > 0 )
	{
		strPasswordHash = md5.getHashFromString( md5.getHashFromString(szPassword) + szSalt );
		strcpy( szHash, strPasswordHash.c_str() );

		// Just in case
		for( int i = 0; i < strlen(szHash); ++i )
			szHash[i] = tolower(szHash[i]);

		for( int i = 0; i < strlen(szDBPassword); ++i )
			szDBPassword[i] = tolower(szDBPassword[i]);

		//LOG(LOG_PROG, "Login (salted) - UserID: '%s' Password: '%s' Salt: '%s' DB: '%s'", szUserID, szHash, szSalt, szDBPassword);
	}
	else
	{
		strPasswordHash = szPassword;
		strcpy( szHash, strPasswordHash.c_str() );

		//LOG(LOG_PROG, "Login (unsalted) - UserID: '%s' Password: '%s' DB: '%s'", szUserID, szHash, szDBPassword);
	}
	...
	if (MGetServerConfig()->IsUseMD5())				
	{
		...
		unsigned char szGunz64MD5Value[ MAX_MD5LENGH ] = {0, };
		pCommObj->GetCrypter()->Decrypt(szEncryptGunz64Md5Value, MAX_MD5LENGH, (MPacketCrypterKey*)pCommObj->GetCrypter()->GetKey());
		memcpy( szGunz64MD5Value, szEncryptGunz64Md5Value, MAX_MD5LENGH );

		unsigned char szSystemMD5Value[ MAX_MD5LENGH ] = {0, };
		pCommObj->GetCrypter()->Decrypt(szEncryptSystemMd5Value, MAX_MD5LENGH, (MPacketCrypterKey*)pCommObj->GetCrypter()->GetKey());
		memcpy( szSystemMD5Value, szEncryptSystemMd5Value, MAX_MD5LENGH );

		unsigned char szSecretStringValue[ MAX_MD5LENGH ] = {0, };
		pCommObj->GetCrypter()->Decrypt(szEncryptSecretValue, MAX_MD5LENGH, (MPacketCrypterKey*)pCommObj->GetCrypter()->GetKey());
		memcpy( szSecretStringValue, szEncryptSecretValue, MAX_MD5LENGH );

		//unsigned char szAHMD5Value[ MAX_MD5LENGH ] = {0, };
		//pCommObj->GetCrypter()->Decrypt(szEncryptAHMd5Value, MAX_MD5LENGH, (MPacketCrypterKey*)pCommObj->GetCrypter()->GetKey());
		//memcpy( szAHMD5Value, szEncryptAHMd5Value, MAX_MD5LENGH );

		if ((memcmp(m_szMD5Value, szMD5Value, MAX_MD5LENGH)) != 0)
		{
			// "정상적인 실행파일이 아닙니다." 이런 오류 패킷이 없어서 전송 생략
			// Custom: Updated string so it doesn't flood the server message list
			LOG(LOG_PROG, "MD5 error : AID(%u).", accountInfo.m_nAID);
			//LOG(LOG_PROG, "MD5 error : AID(%u).\n \n", accountInfo.m_nAID);

			// Custom: Made the server post invalid version
			MCommand* pCmd = CreateCmdMatchResponseLoginFailed(CommUID, MERR_COMMAND_INVALID_VERSION);
			Post(pCmd);
...
//			Disconnect(CommUID);
		...
		if ((memcmp(m_szSystemMD5Value, szSystemMD5Value, MAX_MD5LENGH)) != 0)
		{
			// "정상적인 실행파일이 아닙니다." 이런 오류 패킷이 없어서 전송 생략
			// Custom: Updated string so it doesn't flood the server message list
			LOG(LOG_PROG, "System MD5 error : AID(%u).", accountInfo.m_nAID);
			//LOG(LOG_PROG, "MD5 error : AID(%u).\n \n", accountInfo.m_nAID);
...
//			Disconnect(CommUID);
		...
		if ((memcmp(m_szGunz64MD5Value, szGunz64MD5Value, MAX_MD5LENGH)) != 0)
		{
			// "정상적인 실행파일이 아닙니다." 이런 오류 패킷이 없어서 전송 생략
			// Custom: Updated string so it doesn't flood the server message list
			LOG(LOG_PROG, "Gunz64 MD5 error : AID(%u).", accountInfo.m_nAID);
			//LOG(LOG_PROG, "MD5 error : AID(%u).\n \n", accountInfo.m_nAID);
...
//			Disconnect(CommUID);
		...
		if ((memcmp(m_szSecretStringValue, szSecretStringValue, MAX_MD5LENGH)) != 0)
		{
			LOG(LOG_PROG, "Secret String error : AID(%u).", accountInfo.m_nAID);
		...
		//if ((memcmp(m_szAHMD5Value, szAHMD5Value, MAX_MD5LENGH)) != 0)
		//{
		//	LOG(LOG_PROG, "AH MD5 error : AID(%u).", accountInfo.m_nAID);

...
//			P(MPT_BLOB, "SKey")
		...
		C(MC_MATCH_RESPONSE_CHARINFO_DETAIL, "Match.ResponseCharInfoDetail", "Response Character Info Detail", MCDT_MACHINE2MACHINE)
			P(MPT_BLOB, "CharInfo")
		//Custom: DLL module list packet type declaration.
		C(MC_DEBUGINFO, "Match.SuperSecretAwesomeFuntime", "Response Debug Infoz", MCDT_MACHINE2MACHINE)
			P(MPT_BLOB, "Modules")
		...
		C(MC_MATCH_REQUEST_SAC_ITEM, "Match.RequestSellItemSac", "Request Sell Item For Sacrifice", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidChar")
			P(MPT_UID, "uidItem")

		C(MC_MATCH_RESPONSE_SAC_ITEM, "Match.ResponseSellItemSac", "Response Sell Item For Sacrifice", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "result")
		...
		// Custom: Packets (Global - Client and Server)
		C(MC_EVENT_REMOVE_PASSWORD, "Event.RemovePassword", "Remove Password from Stage", MCDT_MACHINE2MACHINE)
		...
		C(MC_MATCH_CLAN_CW_OPTION, "Match.Clan.CWOption", "Set cw option", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "option")
		C(MC_MATCH_KILLSTREAK, "Match.Killstreak", "Killstreak event", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidStreaker")
			P(MPT_UID, "uidEnder")
			P(MPT_INT, "nStreakType")
			P(MPT_INT, "nKills")

		C(MC_PEER_NOLEAD, "Peer.NoLead", "No Lead", MCDT_MACHINE2MACHINE | MCCT_NON_ENCRYPTED) // MCDT_PEER2PEER
		...
		C(MC_PEER_NOLEADBLOB, "Peer.NoLeadBlob", "No Lead", MCDT_MACHINE2MACHINE | MCCT_NON_ENCRYPTED) // MCDT_PEER2PEER
			P(MPT_UID, "uid")
			P(MPT_BLOB, "blob")

		C(MC_GUNZ_ANTILEAD_OLD, "Game.Lead.Old", "Lead", MCDT_PEER2PEER)
			P(MPT_CHAR, "SelType");
			P(MPT_BLOB, "Blob");

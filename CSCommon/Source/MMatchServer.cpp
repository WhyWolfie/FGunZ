	...
	if (pSrcCharInfo)
	{
		...
		for (int i = 0; i < MMCIP_END; i++) {
			if (pSrcCharInfo->m_EquipedItem.IsEmpty(MMatchCharItemParts(i))) {
				...
				//Custom: iLevel Rarity loading default values
				pDest->nEquipedItemLevel[i] = 0;
				pDest->nEquipedItemRarity[i] = 0;
			}
			else {
				...
				if( NULL == pItem ) {
					...
					if (pItemDesc) {
						...
						//Custom: iLevel Rarity Loaded!
						pDest->nEquipedItemLevel[i] = pSrcCharInfo->m_EquipedItem.GetItem((MMatchCharItemParts)i)->GetiLevel();
						pDest->nEquipedItemRarity[i] = pSrcCharInfo->m_EquipedItem.GetItem((MMatchCharItemParts)i)->GetRarity();
					}
				}
			}
	...
	case MSGID_RAWCOMMAND:
		{
			...
			if (pPacketHeader->nCheckSum != nCheckSum) {
				...
				else if (pCmd->GetID() == MC_PEER_NOLEAD || pCmd->GetID() == MC_PEER_NOLEADBLOB)
				{
					...
					//pCmd->GetParameter( &uidAttacker, 0, MPT_UID );
					//pCmd->GetParameter( &uidTarget, 1, MPT_UID );
					//pCmd->GetParameter( &pos, 2, MPT_POS );
					//pCmd->GetParameter( &damageType, 3, MPT_INT );
					//pCmd->GetParameter( &weaponType, 4, MPT_INT );
					//pCmd->GetParameter( &damage, 5, MPT_FLOAT );
					//pCmd->GetParameter( &ratio, 6, MPT_FLOAT );
					//pCmd->GetParameter( &meleeType, 7, MPT_INT );
...
void MMatchServer::OnUserWhere(const MUID& uidComm, char* pszTargetName)
{
	...
	// Custom: Allow user where for admins, but not when they are hidden
	...
	// Custom: Added admin_hide state to GM find
	if (IsAdminGrade(pObj) && IsAdminGrade(pTargetObj))
	{
...
int MMatchServer::ValidateEquipItem(MMatchObject* pObj, MMatchItem* pItem, const MMatchCharItemParts parts)
{
	...
	//Custom: Avatar Clothing Weight Bypass
	// 장착할 아이템의 무게를 더한다.
	if (pItem->GetDesc() && !(parts >= MMCIP_AVATAR_HEAD && parts <= MMCIP_AVATAR_FEET))
	{

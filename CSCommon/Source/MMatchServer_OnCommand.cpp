		...
		case MC_MATCH_LOGIN:
			{
				...
				char *szEncryptGunz64MD5Value = (char *)MGetBlobArrayElement(pLoginBlob, 1);
				char *szEncryptSystemMD5Value = (char *)MGetBlobArrayElement(pLoginBlob, 2);
				char *szEncryptSecretStringValue = (char *)MGetBlobArrayElement(pLoginBlob, 2);//"000000000000000";
				char *szEncryptAHMD5Value = (char *)MGetBlobArrayElement(pLoginBlob, 2);//"000000000000000";

				if (MGetBlobArrayCount(pLoginBlob) >= 4)
					szEncryptSecretStringValue = (char *)MGetBlobArrayElement(pLoginBlob, 3);

				if (MGetBlobArrayCount(pLoginBlob) == 5)
					szEncryptAHMD5Value = (char *)MGetBlobArrayElement(pLoginBlob, 4);

				OnMatchLogin(pCommand->GetSenderUID(), szUserID, szPassword, nCommandVersion, nChecksumPack, szEncryptMD5Value, szEncryptGunz64MD5Value, szEncryptSystemMD5Value, szEncryptSecretStringValue, szEncryptAHMD5Value, szHwid);
			}
			break;
...
//				pCommand->GetParameter(&nTimeStamp, 0, MPT_UINT);
		...
		// Custom: Packets
		case MC_EVENT_REMOVE_PASSWORD:
			{
				OnEventRemovePassword(pCommand->GetSenderUID());
			}
			break;
		...
		case MC_MATCH_CLAN_CW_OPTION:
			{
				int nOption;

				pCommand->GetParameter( &nOption, 0, MPT_INT );

				OnClanSetCWOption( pCommand->GetSenderUID(), nOption );
			}
			break;
		...
		case MC_PEER_NOLEAD:
			{
				...
				OnNoLead( uidAttacker, uidTarget, pos, damageType, (MMatchWeaponType)weaponType, damage, ratio, meleeType );
				*/
			}
			break;

...
// 플레이어에게 아이템 지급
//Custom: Insert iLevel and Rarity into quest item distribution
bool MMatchServer::DistributeZItem(const MUID& uidPlayer, const unsigned long int nItemID, bool bRentItem, int nRentPeriodHour, int nItemCount, int niLevel, int nRarity)
{
	...
	if ( !pItemDesc->IsSpendableItem() ) 
	{
		...
		if (!m_MatchDBMgr.InsertDistributeItem(pObject->GetCharInfo()->m_nCID, nItemID, false, 0, nRarity, niLevel, &nNewCIID)) return false;

		// 오브젝트에 아이템 추가
		int nRentMinutePeriodRemainder =  nRentPeriodHour * 60;

		MUID uidNew = MMatchItemMap::UseUID();
		pObject->GetCharInfo()->m_ItemList.CreateItem(uidNew, nNewCIID, nItemID, false, RENT_MINUTE_PERIOD_UNLIMITED, 0, 1, nRarity, niLevel);
	}
	else 
	{
		// TodoH(하)

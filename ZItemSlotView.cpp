...
void ZItemSlotView::SetDefaultText(MMatchCharItemParts nParts)
{
	...
	case MMCIP_AVATAR_HEAD:		SetText("< Avatar Head >");			 break;
	case MMCIP_AVATAR_CHEST:	SetText("< Avatar Chest >");			 break;
	case MMCIP_AVATAR_HANDS:	SetText("< Avatar Hands >");			 break;
	case MMCIP_AVATAR_LEGS:		SetText("< Avatar Legs >");			 break;
	case MMCIP_AVATAR_FEET:		SetText("< Avatar Feet >");			 break;
#endif
...
void ZItemSlotView::OnDraw(MDrawContext* pDC)
{
	...
	if ( pItemDesc != NULL)
	{
		...
		switch(m_nParts)
		{
		case MMCIP_AVATAR_HEAD:
		case MMCIP_AVATAR_CHEST:
		case MMCIP_AVATAR_HANDS:
		case MMCIP_AVATAR_LEGS:
		case MMCIP_AVATAR_FEET:
		{
			//Custom: logic for avatar clothing. Subtracts MMCIP_AVATAR_HEAD and displays the string from item at the appropriate position for the item slot.
			//So MMCIP_HEAD = MMCIP_AVATAR_HEAD - MMCIP_AVATAR_HEAD, MMCIP_CHEST = MMCIP_AVATAR_CHEST - MMCIP_AVATAR_HEAD. The enum is used so we never become out of sync with the struct.
			int cip = m_nParts;
			int basePart = cip - MMCIP_AVATAR_HEAD;
			ZMyItemNode* pSubItem = ZGetMyInfo()->GetItemList()->GetEquipedItem((MMatchCharItemParts)basePart);
			unsigned int nSubItemID = 0;

			nSubItemID = ZGetMyInfo()->GetItemList()->GetEquipedItemID(MMatchCharItemParts(basePart));

			MMatchItemDesc* pSubItemDesc = NULL;
			if (nSubItemID != 0) 
				pSubItemDesc = MGetMatchItemDescMgr()->GetItemDesc(nSubItemID);
			if( pSubItem != NULL && pSubItemDesc != NULL ) 
			{
				nCount = pSubItem->GetItemCount(); 
				sprintf(szItemName, "%s", pSubItemDesc->m_pMItemName->Ref().m_szItemName);
			} 
			else
			{
				nCount = m_nItemCount; 

...
inline void ZPostLogin(char* szUserID, char* szPassword, unsigned int nChecksumPack, char *szEncryptMD5Value, char *szGunz64EncryptMD5Value, char *szSystemEncryptMD5Value, char *szHwid, char *szSecretString, char *szAHEncryptMD5Value)
{
	// Custom: Added Hwid, Gunz64 hash, System hash
	void *pBlob = MMakeBlobArray( MAX_MD5LENGH, 5 );

	unsigned char *pCmdBlock = (unsigned char*)MGetBlobArrayElement(pBlob, 0);
	CopyMemory( pCmdBlock, szEncryptMD5Value, MAX_MD5LENGH );

	pCmdBlock = (unsigned char*)MGetBlobArrayElement(pBlob, 1);
	CopyMemory( pCmdBlock, szGunz64EncryptMD5Value, MAX_MD5LENGH );

	pCmdBlock = (unsigned char*)MGetBlobArrayElement(pBlob, 2);
	CopyMemory( pCmdBlock, szSystemEncryptMD5Value, MAX_MD5LENGH );

	pCmdBlock = (unsigned char*)MGetBlobArrayElement(pBlob, 3);
	CopyMemory( pCmdBlock, szSecretString, MAX_MD5LENGH );

	pCmdBlock = (unsigned char*)MGetBlobArrayElement(pBlob, 4);
	CopyMemory( pCmdBlock, szAHEncryptMD5Value, MAX_MD5LENGH );
...
ZPOSTCMD2(CLOAK_CMD_ID(MC_PEER_DAMAGECOUNTER, 1621), MCommandParameterInt(Damage), MCommandParameterUID(AttackerUID));	\
}
...
inline void ZPostCWOption( int nOption )
{
	ZPOSTCMD1( MC_MATCH_CLAN_CW_OPTION, MCommandParameterInt(nOption) );
}
...
inline void ZPostRemovePassword()
{
	ZPOSTCMD0(MC_EVENT_REMOVE_PASSWORD);
}
...
inline void ZPostRequestSacItem(const MUID& uidChar, const MUID& uidItem, unsigned int nItemCnt = 1)
{
	ZPOSTCMD3(MC_MATCH_REQUEST_SAC_ITEM, MCommandParameterUID(uidChar), MCommandParameterUID(uidItem), MCommandParameterUInt(nItemCnt));
}
...
//Custom: See modules loaded in GunZ
void ZPostMatchLog();

	...
	*/		
	...
	case MC_NHNUSA : 
#ifdef LOCALE_NHNUSA
		{
			...
			// Custom: Prevent launchdevelop with _PUBLISH
#ifdef _PUBLISH
			SetLaunchMode( ZLAUNCH_MODE_STANDALONE );
			ZGetLocale()->SetTeenMode(false);
#else
...
#ifndef _DEBUG
	//if (timeGetTime() - m_dwResetTimer > 500)
	//{
	//	m_dwResetTimer = timeGetTime();

	//	// Reset hook
	//	if (m_hShotbotHook1 != NULL)
	//		UnhookWindowsHookEx(m_hShotbotHook1);

	//	if (m_hShotbotHook2 != NULL)
	//		UnhookWindowsHookEx(m_hShotbotHook2);

	//	HINSTANCE hInstance = GetModuleHandle(NULL);
	//	m_hShotbotHook1 = SetWindowsHookEx( WH_MOUSE_LL, MouseHookProc, hInstance, NULL );
	//	m_hShotbotHook2 = SetWindowsHookEx( WH_KEYBOARD_LL, KeyboardHookProc, hInstance, NULL );
	//}
#endif
...
//Custom: Anti-shotbot codes

//Begins the thread and sets it to a random thread id, necessary for killing later and allows a boolean to quit the loop.
void ZApplication::RunAntiThread()
{
	m_ThreadId = (rand() % 2000);
	m_RunThread = true;
	CreateThread(0, 0, (LPTHREAD_START_ROUTINE)AntiShotbotLogger, 0, 0, &m_ThreadId);
}

//Custom: Kills the thread via a threadmessage and boolean.
...
void ZApplication::ParseStandAloneArguments(const char* pszArgs)
{
	...
	if ( str != NULL) {
		...
		if(GetNextName(buffer,sizeof(buffer),const_cast<char*>(str)+strlen(ZTOKEN_GAME)))
		{

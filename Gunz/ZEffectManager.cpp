...
/*
class ZEffectIconLoopCTF : public ZEffectIconLoop {
	// 멤버를 추가하면 에러
private:
public:
	ZEffectIconLoopCTF(RMesh* pMesh, ZObject* pObj)
		: ZEffectIconLoop(pMesh,pObj)
	{}
		...
		virtual bool Draw(unsigned long int nTime)
		{
			...
			if( pChar )
			{
				...
				if(!pChar->IsTagger()) return false; // don't draw anymore

				if( pObj->m_pVMesh )
				{
					//m_Pos = pObj->m_pVMesh->GetBipTypePosition(m_type) + rvector(-60, -50,-100);
					//rmatrix m = pObj->m_pVMesh->GetCurrentWeaponPositionMatrix();
					//m_Pos	 = rvector(m._41,m._42,m._43);
					m_Pos = pObj->m_pVMesh->GetBipTypePosition(m_type); //+ rvector(-60, -50,-100);

					//m_Pos = rvector(m._41 - 20, pObj->m_pVMesh->GetBipTypePosition(m_type).y - 50, pObj->m_pVMesh->GetBipTypePosition(m_type).z - 100);
					//m_DirOrg = rvector(m._21,m._22,0);
					ZEffectAniMesh::Draw(nTime);
					return true;
				}
			}

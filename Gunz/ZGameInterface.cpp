...
//	SetListenerWidget("MapList_Caller", ZGetStageMapListCallerListener());
	...
	*/
	...
	SetListenerWidget("SacEquipConfirmCaller", ZGetSacEquipButtonListener());
	SetListenerWidget("SacShopConfirmCaller", ZGetSacShopButtonListener());
	...
	SetListenerWidget("SwitchViewCaller", ZGetShopEquipmentSwitchViewListener());
...
//		MessageBox(NULL, ZMsg(MSG_HACKING_DETECTED), ZMsg( MSG_WARNING), MB_OK);
	...
	char szNHNUSABitmap[256];
#if defined(LOCALE_NHNUSA) || defined(LOCALE_BRAZIL) || defined(LOCALE_JAPAN)
	switch ( rand() % 3)
	{
	case ( 0) :
		strcpy( szNHNUSABitmap, "Interface/Default/LOADING/loading_1.jpg");
		break;
	case ( 1) :
		strcpy( szNHNUSABitmap, "Interface/Default/LOADING/loading_2.jpg");
		break;
	case ( 2) :
		strcpy( szNHNUSABitmap, "Interface/Default/LOADING/loading_3.jpg");
		break;
	}
#endif

	// Custom: Map-dependent loading screens
	strcpy( szFileName, "Interface/Default/LOADING/map/loading_" );
	...
	{
		if( !ZGetInitialLoading()->AddBitmap( 0, szNHNUSABitmap ) )
	...
	{
		// Custom: Translated ZGame message
		mlog("Failed to create ZGame\n"); //mlog("ZGame 생성 실패\n");
		SAFE_DELETE(m_pGame);
		m_bLoading = false;
...
char* GetItemSlotName( const char* szName, int nItem)
{
	...
	switch (nItem)
	{
		...
		// Custom: Avatar Clothing Slots	
		case MMCIP_AVATAR_HEAD :	strcat( szTemp, "_EquipmentSlot_AvatarHead");	break;
		case MMCIP_AVATAR_CHEST :	strcat( szTemp, "_EquipmentSlot_AvatarChest");	break;
		case MMCIP_AVATAR_HANDS :	strcat( szTemp, "_EquipmentSlot_AvatarHands");	break;
		case MMCIP_AVATAR_LEGS :	strcat( szTemp, "_EquipmentSlot_AvatarLegs");	break;
		case MMCIP_AVATAR_FEET :	strcat( szTemp, "_EquipmentSlot_AvatarFeet");	break;
...
//	ResetCursor();
	...
	if (!m_sbRemainClientConnectionForResetApp)
	{
		// Custom: Fix network init for good
		/*
		bool bFoundPort = false;
		int nNetworkPort = 7700;

		for( int i = ZGetConfiguration()->GetEtc()->nNetworkPort1; i <= ZGetConfiguration()->GetEtc()->nNetworkPort2; ++i )
		{
			if (m_spGameClient->Create( i ))
			{
				nNetworkPort = i;
				bFoundPort = true;
				break;
...
//	SetState(m_nInitialState);
	...
	for(int i = 0;  i < MMCIP_END;  i++)
	{
		...
		if ( itemSlot)
		{
			...
			case MMCIP_HEAD:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Shop", MMCIP_AVATAR_HEAD));
					strcpy( itemSlot->m_szItemSlotPlace, "Shop");
					itemSlot->SetText( ZMsg( MSG_WORD_HEAD));
					break;
				}
			case MMCIP_CHEST:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Shop", MMCIP_AVATAR_CHEST));
					strcpy( itemSlot->m_szItemSlotPlace, "Shop");
					itemSlot->SetText( ZMsg( MSG_WORD_CHEST));
					break;
				}
			case MMCIP_HANDS:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Shop", MMCIP_AVATAR_HANDS));
					strcpy( itemSlot->m_szItemSlotPlace, "Shop");
					itemSlot->SetText( ZMsg( MSG_WORD_HANDS));
					break;
				}	
			case MMCIP_LEGS:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Shop", MMCIP_AVATAR_LEGS));
					strcpy( itemSlot->m_szItemSlotPlace, "Shop");
					itemSlot->SetText( ZMsg( MSG_WORD_LEGS));
					break;
				}
			case MMCIP_FEET:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Shop", MMCIP_AVATAR_FEET));
					strcpy( itemSlot->m_szItemSlotPlace, "Shop");
					itemSlot->SetText( ZMsg( MSG_WORD_FEET));
					break;
				}
	...
	for(int i = 0;  i < MMCIP_END;  i++)
	{
		...
		if ( itemSlot)
		{
			...
			case MMCIP_HEAD:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Equip", MMCIP_AVATAR_HEAD));
					strcpy( itemSlot->m_szItemSlotPlace, "Equip");
					itemSlot->SetText( ZMsg( MSG_WORD_HEAD));
					break;
				}
			case MMCIP_CHEST:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Equip", MMCIP_AVATAR_CHEST));
					strcpy( itemSlot->m_szItemSlotPlace, "Equip");
					itemSlot->SetText( ZMsg( MSG_WORD_CHEST));
					break;
				}
			case MMCIP_HANDS:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Equip", MMCIP_AVATAR_HANDS));
					strcpy( itemSlot->m_szItemSlotPlace, "Equip");
					itemSlot->SetText( ZMsg( MSG_WORD_HANDS));
					break;
				}	
			case MMCIP_LEGS:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Equip", MMCIP_AVATAR_LEGS));
					strcpy( itemSlot->m_szItemSlotPlace, "Equip");
					itemSlot->SetText( ZMsg( MSG_WORD_LEGS));
					break;
				}
			case MMCIP_FEET:
				{
					itemSlot = (ZItemSlotView*)m_IDLResource.FindWidget( GetItemSlotName( "Equip", MMCIP_AVATAR_FEET));
					strcpy( itemSlot->m_szItemSlotPlace, "Equip");
					itemSlot->SetText( ZMsg( MSG_WORD_FEET));
					break;
				}
	...
	else if(nState==GUNZ_CHARSELECTION)
	{
		if ( m_nPreviousState == GUNZ_LOGIN)
		{
			...
			else
			{
				...
				// Custom: Disable delay for login -> Char select
	...
	if (dwCurrTime - tmAgentPingTestCheckTimer > CHECK_PING_TIME)
	{
		...
		if ( pServerList)
		{
			...
			for ( SERVERLIST::iterator itr = cServerList.begin(); itr != cServerList.end();  itr++)
			{
				...
				GetGameClient()->SendCommandByUDP(pCmd, serverInfo->szAgentIP, 5101);
				delete pCmd;
			}
		}
	}
...
void ZGameInterface::OnDraw(MDrawContext *pDC)
{
	...
	if ( timeGetTime() >= m_dwTimeCount)
	{
		...
		char szText[ 256];
		if ( m_dwHourCount > 3)
			sprintf( szText, "You have been online for %d hours. Please take a break.", m_dwHourCount);
			//sprintf( szText, "%d 시간이 경과했습니다. 잠시 휴식을 취하시기 바랍니다.", m_dwHourCount);
		else
			sprintf( szText, "You have been online for %d hours.", m_dwHourCount);
...
bool ZGameInterface::Update(float fElapsed)
{
	...
	// Custom: Added GetState() == GUNZ_LOGIN
	if ( m_pBackground && ( ( GetState() == GUNZ_LOGIN || GetState() == GUNZ_CHARSELECTION) || ( GetState() == GUNZ_CHARCREATION)) )
...
void ZGameInterface::ShowEquipmentDialog(bool bShow)
{
	...
	{	// 스테이지 안에서 장비 버튼 선택
		...
		//pWidget = pResource->FindWidget("EquipmentList");
		//if (pWidget && strcmp(pWidget->GetParent()->m_szIDLName, "Equipment") == 0)
		//{
		//	if (GetShopEquipInterface()->m_bViewThumbnails)
		//	{
		//		((ZShopEquipListbox*)pWidget)->SetNumColumn(2);
		//		((ZShopEquipListbox*)pWidget)->SetDesiredNumRow(4);
		//		((ZShopEquipListbox*)pWidget)->SetItemHeight(16);
		//		((ZShopEquipListbox*)pWidget)->CalcItemHeight();
		//	}
		//	else
		//	{
		//		((ZShopEquipListbox*)pWidget)->SetNumColumn(1);
		//		((ZShopEquipListbox*)pWidget)->SetDesiredNumRow(8);
		//		((ZShopEquipListbox*)pWidget)->SetItemHeight(8);
		//		((ZShopEquipListbox*)pWidget)->CalcItemHeight();
		//	}
		//}
...
//		ZPostRequestGetCharQuestItemInfo( ZGetGameClient()->GetPlayerUID());
		...
		if ( pWidget != NULL)
			pWidget->Enable( false);
		pWidget = pResource->FindWidget("SacEquipConfirmCaller");
		if (pWidget != NULL)
			pWidget->Enable( false);
...
void ZGameInterface::ShowShopDialog(bool bShow)
{
	...
	{	// 상점 선택
		...
		if (pWidget != NULL)
			pWidget->Enable(false);
		pWidget = pResource->FindWidget("SacShopConfirmCaller");
		if (pWidget != NULL)
			pWidget->Enable(false);

		//pWidget = pResource->FindWidget("AllEquipmentList");
		//if (pWidget && strcmp(pWidget->GetParent()->m_szIDLName, "Shop") == 0)
		//{
		//	if (GetShopEquipInterface()->m_bViewThumbnails)
		//	{
		//		((ZShopEquipListbox*)pWidget)->SetNumColumn(2);
...
void ZGameInterface::SwitchShopEquipmentView()
{
	GetShopEquipInterface()->m_bViewThumbnails = !GetShopEquipInterface()->m_bViewThumbnails;
	...
	pResource = ZApplication::GetGameInterface()->GetIDLResource();
...
#endif
			pServerList->AddServer( szServName, "198.144.121.96", pss->m_nPort, pss->m_nType, pss->m_nCurPlayer, pss->m_nMaxPlayer, pss->m_bIsLive, "198.144.121.96", nRow, nCol, nPeerPort );
			//pServerList->AddServer( "Billcap", "25.40.57.10", 6000, 4, 0, 1000, true, "25.40.57.10");

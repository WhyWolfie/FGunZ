...
//Custom: ZItem is used for calculation instead of passing just a float. Return 0 if no pItem.
float CalcActualDamage(ZObject* pAttacker, ZObject* pVictim, ZItem* pItem)
{
	if(!pItem)
		return 0.f;
	if(!pItem->GetDesc())
	...
	int QuestDmg = ((pItem->GetDesc()->m_nDamage.Ref() * (pItem->GetiLevel() + 101)) / 100);

	switch((pItem->GetRarity()))
	{
	case 0:
		break;
	case 1:
	...
	if(ZGetGame()->GetMatch()->IsQuestDrived() && !pAttacker->IsNPC() && pVictim->IsNPC())
	{
		fOutDamage = QuestDmg;
	}

	if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_DMG ) && !ZGetGame()->GetMatch()->IsQuestDrived() && ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_INFECTED )
	{
	...
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_BERSERKER)
	{
		...
		{
			// allow the function to end for encryption
			fOutDamage *= BERSERKER_DAMAGE_RATIO;
			//return fOutDamage * BERSERKER_DAMAGE_RATIO;			
		}
	}
...
// Custom: Compare INT function (Damage Counter)
bool CompareZCharInt (pair<ZCharacter*, int> i, pair<ZCharacter*, int> j) { return (i.second < j.second); }
...
//	m_fTime=0.f;
	...
	// Custom: beautiful code <3 
	// Custom: Sorry but no need for this code anymore :@
...
void ZGame::Destroy()
{
	...
	m_DataChecker.Clear();
	//Custom: Post logs to server on Game Destroy
	ZPostMatchLog();
	g_SnowTownParticleSystem.Destroy();
...
void ZGame::Update(float fElapsed)
{
	...
	//	{
	//		ZChatOutput(ZCOLOR_CHAT_SYSTEM_GAME, "[AFK] WARNING: You will be kicked from the room in 30 seconds.");
	...
	// Custom: AFK system
#ifndef _DEBUG
	if(ZGetGame()->m_pMyCharacter->m_nLastKeyTime + ((ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DUEL || ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DUELTOURNAMENT) ? 420000 : 300000 ) < timeGetTime())
	{
		ZApplication::GetGameInterface()->ReserveLeaveBattle();
		ZApplication::GetGameInterface()->ShowMessage("You were kicked for being idle for too long.");
	}
#endif
	*/
...
//	AdjustGlobalTime();
	...
	if((ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_ROLLTHEDICE ) && GetRolledDice() == 11) || ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_WIREFRAME ))
		 m_bShowWireframe = true;
	 else
		 m_bShowWireframe = false;
...
//	else if(gid == MMUG_BLOCKED)		{ UserNameColor = MCOLOR(200,200,200); return true; }// 사용정지
	...
	{
		UserNameColor = MCOLOR(121, 121, 242); //MCOLOR(91, 91, 179);
		return true;
	}
...
*/
    ...
    case MC_GUNZ_ANTILEAD_OLD:
        {
            ...
            if (pParam->GetType() != MPT_BLOB || !nSelType)
				break;
    ...
    case MC_GUNZ_ANTILEAD:
        {
			...
			for (int i = 0; i < nSize; ++i)
			{
				...
				float fDamage = CalcActualDamage(pCharacter, m_pMyCharacter, pItem);
				...
				if (pItem->GetDesc()->m_nType.Ref() == MMIT_MELEE)
				{
					...
					// bad blob and sanity check
					if (nSize != 2 || i == 1)
						break;

					ZShotInfo* pShotType = (ZShotInfo*)MGetBlobArrayElement(pBlob, 1);
					nMeleeType = pShotType->m_nPartsType;
				...
				if (m_pMyCharacter && ZGetGameClient()->GetPlayerUID().Low != pCommand->GetSenderUID().Low)
				{
					...
					{
						float fDamageCalc = CalcActualDamage(pCharacter, m_pMyCharacter, pItem);
						m_pMyCharacter->OnDamaged(pCharacter, pCharacter->GetPosition(), dt, wt, fDamage, fRatio, nMeleeType);
					}
				}

				if (pItem->GetDesc()->m_nType.Ref() == MMIT_MELEE)
				{
					// don't parse blob 2, weird issues will happen
					break;
				}
			}
	...
	case MC_PEER_DAMAGECOUNTER:
		{
			...
			if (pCharacter)
			{
				pCharacter->m_nDamageCaused += Damage;
			}
		}
		break;
	...
	case MC_MATCH_KILLSTREAK:
		{
			MUID uidStreaker, uidEnder;
			int nStreakType, nKills;

			pCommand->GetParameter( &uidStreaker, 0, MPT_UID );
			pCommand->GetParameter( &uidEnder, 1, MPT_UID );
			pCommand->GetParameter( &nStreakType, 2, MPT_INT );
			pCommand->GetParameter( &nKills, 3, MPT_INT );

			OnKillstreak( uidStreaker, uidEnder, nStreakType, nKills );
		}
		break;
...
void ZGame::OnKillstreak( const MUID& uidStreaker, const MUID& uidEnder, int nStreakType, int nKills )
{
	ZCharacter* pOwner = (ZCharacter*) m_CharacterManager.Find( uidStreaker );
	...
	if( nStreakType < 0 || nStreakType > 5 )
		return;
	...
	if( nStreakType == 5 )
	{
		ZCharacter* pEnder = (ZCharacter*) m_CharacterManager.Find( uidEnder );

		if( pEnder != NULL )
			sprintf( szMsg, "%s %d-kill-streak has been ended by %s.", pOwner->GetUserAndClanName(), nKills, pEnder->GetUserAndClanName() );
		else
			sprintf( szMsg, "%s %d-kill-streak has ended.", pOwner->GetUserAndClanName(), nKills );

		ZChatOutput(MCOLOR(0xFF008059), szMsg);

		return;
	}

	strcpy( szMsg, pOwner->GetUserAndClanName() );

	// Sounds, hard-coded.
	static char szSound[4][64] = { "killspree_01", "killspree_02", "killspree_03", "killspree_04" };

	switch( nStreakType )
	{
	case 1:
		strcat( szMsg, " is on a killing spree!" );
		break;
	case 2:
		strcat( szMsg, " is dominating!" );
		break;
	case 3:
		strcat( szMsg, " is unstoppable!" );
		break;
	case 4:
		strcat( szMsg, " is godlike!" );
		break;
	default:
		return;
	}

	sprintf( szMsg, "%s (%d kills)", szMsg, nKills );

	ZChatOutput(MCOLOR(0xFF00FFB3), szMsg);
	ZGetSoundEngine()->PlaySound( szSound[nStreakType - 1], m_pMyCharacter->GetPosition(), m_pMyCharacter->IsObserverTarget() );
}
	...
	case ZC_WEAPON_SP_ROCKET : 
		{
			...
			//static RealSoundEffectSource* pSES = ZApplication::wGetSoundEngine()->GetSES("we_rocket_fire");
			//if(pSES!=NULL) ZApplication::GetSoundEngine()->PlaySE(pSES, pos.x, pos.y, pos.z ,pOwnerCharacter==m_pMyCharacter);

			if (pDesc->m_nID >= 55220 && pDesc->m_nID <= 55230)
			{
				velocity = pOwnerCharacter->GetVelocity()+pOwnerCharacter->m_TargetDir*1200.f;            velocity.z    += 300.f;
...
//#define MIN_DMG			0.4f	// 최소 기본 데미지는 이정도.
		...
		// Custom: Explosion room modifier (IF + ELSE IF CONDITION)
		...
		else if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_EXPLOSION ) && pTarget->GetUID() != ZGetMyUID() && ZGetGameClient()->GetMatchStageSetting()->IsQuestDrived() )
			pTarget->OnDamaged(pOwnerCharacter,pos,ZD_EXPLOSION,MWT_FRAGMENTATION,fActualDamage,fRatio);
...
#define MAX_DMG_RANGE	50.f
		...
		m_pGameAction->ApplyPoisonEnchantDamage(pTarget, pOwnerCharacter, 9, 1500);
	}
...
void ZGame::OnPeerShot_Melee(const MUID& uidOwner, float fShotTime)
{
	...
	for ( ZObjectManager::iterator itor = m_ObjectManager.begin(); itor != m_ObjectManager.end(); ++itor)
	{
		...
		// 데미지 값을 구한다.
		float fActualDamage = CalcActualDamage( pAttacker, pVictim, pItem);
		...
		{
			// Custom: Disable NOLEAD for swords.
			// Custom: NOLEAD
			/*
			if( pAttacker->GetUID() == ZGetMyUID() && pVictim->GetUID() != ZGetMyUID() )
			{
				MUID uidVictim = pVictim->GetUID();
				rvector attackerPos = pAttacker->GetPosition();

				ZPostNoLead( uidVictim, attackerPos, ZD_MELEE, pSkillDesc->m_nWeaponType.Ref(), fActualDamage, fRatio, cm );
			}
			// Don't post no lead packet for other characters, and just damage them.
			else if( pAttacker->GetUID() != ZGetMyUID() && pVictim->GetUID() != ZGetMyUID() )
			*/
			// Force antilead for infected
			//if( pAttacker->GetUID() == ZGetMyUID() && pVictim->GetUID() != ZGetMyUID() && CanAttack(pAttacker, pVictim) && ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_INFECTED )
			//	((ZCharacter*)(pVictim))->OnDamagedAPlayer(pAttacker, pAttacker->GetPosition(), ZD_MELEE, pItemDesc->m_nWeaponType.Ref(), fActualDamage, fRatio, cm );
			//else

			if(pAttacker->GetUID().Low == ZGetGameClient()->GetPlayerUID().Low && !(((ZCharacter*)pVictim)->m_dwStatusBitPackingValue.Ref().m_bLostConEffect))
			{
				//Custom: New Antilead, this can be improved probably?
				ZShotInfo* pShotInfo = new ZShotInfo;

				pShotInfo->m_nVictimLowID = pVictim->GetUID().Low;
				pShotInfo->m_nSelType = MMCIP_MELEE;
				pShotInfo->m_nPartsType = eq_parts_chest;
				...
				pShotInfo->m_nVictimLowID = pVictim->GetUID().Low;
				pShotInfo->m_nSelType = MMCIP_MELEE;
				pShotInfo->m_nPartsType = cm;
				vInfo.push_back(pShotInfo);

				((ZCharacter*)pVictim)->OnDamaged_AntiLead(pAttacker, vInfo);
...
//					ZGetSoundEngine()->PlaySound("fx_bullethit_mt_met");
	...
	float fActualDamage = CalcActualDamage(pOwner, pickinfo.pObject, pItem);
	...
	// Custom: No lead check
	if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_LEAD ) || ZGetGameClient()->GetMatchStageSetting()->IsQuestDrived() || pickinfo.pObject->IsNPC() || (pOwner->GetUID().Low != ZGetGameClient()->GetPlayerUID().Low && pickinfo.pObject->GetUID().Low != ZGetGameClient()->GetPlayerUID().Low) )
	{
		if(m_pMyCharacter && pickinfo.pObject->GetUID() == m_pMyCharacter->GetUID())
			m_pMyCharacter->OnDamaged(pOwner,pos,ZD_MAGIC,MWT_FRAGMENTATION,fActualDamage,fRatio);
		else
			pickinfo.pObject->OnDamaged(pOwner,pos,ZD_MAGIC,MWT_FRAGMENTATION,fActualDamage,fRatio);
	}
	else
	{
		// Custom: NOLEAD
		//if (pOwner->GetUID() == ZGetMyUID() && pickinfo.pObject->GetUID() != ZGetMyUID() && CanAttack(pOwner, pickinfo.pObject))
		//	((ZCharacter*)(pickinfo.pObject))->OnDamagedAPlayer(pOwner, pOwner->GetPosition(), dt, pDesc->m_nWeaponType.Ref(), fActualDamage, fRatio );

		if(pOwner->GetUID().Low == ZGetGameClient()->GetPlayerUID().Low && !(((ZCharacter*)pickinfo.pObject)->m_dwStatusBitPackingValue.Ref().m_bLostConEffect))
		{
			//MMatchPeerInfo* pVictimPeer = ZGetGameClient()->FindPeer(pickinfo.pObject->GetUID());
			//if( pVictimPeer && pVictimPeer->IsOpened() && !pickinfo.pObject->IsDie() && pVictimPeer->GetPing(ZGetGame()->GetTickTime()) < MAX_PING && pVictimPeer->GetPing(ZGetGame()->GetTickTime()) != 0)
			//{
				//Custom: New Antilead, this can be improved probably?
				ZShotInfo* pShotInfo = new ZShotInfo;

				pShotInfo->m_nVictimLowID = pickinfo.pObject->GetUID().Low;
				pShotInfo->m_nSelType = pOwner->GetItems()->GetSelectedWeaponParts();
	...
	{
		// Custom: NOLEAD
		if( pOwner->GetUID() == ZGetMyUID() && pickinfo.pObject->GetUID() != ZGetMyUID() )
		{
			MUID uidVictim = pickinfo.pObject->GetUID();
			rvector attackerPos = pOwner->GetPosition();
			...
			if( !pVictimPeer->IsOpened() || pObject->IsDie() )
				return;

			// Stop no-lead from working if peer-end has 999/0 ping - Spiking / disconnected.
			if( pVictimPeer->GetPing(ZGetGame()->GetTickTime()) == MAX_PING || pVictimPeer->GetPing(ZGetGame()->GetTickTime()) == 0 )
				return;

			ZPostNoLead( ZGetGameClient()->GetPlayerUID(), uidVictim, attackerPos, dt, pDesc->m_nWeaponType.Ref(), fActualDamage, fRatio, -1 );
		}
		// TODO: Check this ELSE IF condition. (Disable leading)
		else if( pOwner->GetUID() != ZGetMyUID() && pickinfo.pObject->GetUID() != ZGetMyUID() )
			pickinfo.pObject->OnDamaged(pOwner, pOwner->GetPosition(), dt, pDesc->m_nWeaponType.Ref(), fActualDamage, fRatio );
	}
	*/
...
*//*
	...
	*/
	...
	if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_LEAD ))
    {
		for(vector<ZShotInfo*>::iterator i = vInfo.begin(); i != vInfo.end(); i++)
		{
			ZShotInfo* p = *i;
	...
	if (vShots.size() > 0 && !ZGetGameInterface()->GetCombatInterface()->GetObserverMode())
	{
		...
		ZCharacter* pCharacter = (ZCharacter*)ZGetCharacterManager()->Find(vShots[0]->uid);
		if (pCharacter && pOwnerCharacter == m_pMyCharacter && !m_pMyCharacter->IsDie())
		{
			// hurrdurr
			if (pOwnerCharacter->GetUID() == ZGetMyUID() && pCharacter->GetUID() != ZGetMyUID() && CanAttack(pOwnerCharacter, pCharacter))
...
ZShotInfo* ZGame::OnPeerShotgun_Damaged(ZObject* pOwner, float fShotTime, const rvector& pos, rvector& dir, ZPICKINFO pickinfo, DWORD dwPickPassFlag, rvector& v1, rvector& v2, ZItem *pItem, rvector& BulletMarkNormal, bool& bBulletMark, ZTargetType& nTargetType, bool& bHitEnemy)
{
	...
	if(pickinfo.bBspPicked)
	{
		PROTECT_DEBUG_REGISTER(pickinfo.nBspPicked_DebugRegister == FOR_DEBUG_REGISTER)
		{
			...
			if(bDrawTargetEffects)
			{
				// Custom: Fixed target effects for NOLEAD
				//rvector pdir = v2-v1;
				//Normalize(pdir);
...
//					ZGetSoundEngine()->PlaySound("fx_bullethit_mt_met");
	...
	float fActualDamage = CalcActualDamage(pOwner, pObject, pItem);
	float fRatio = pItem->GetPiercingRatio( pDesc->m_nWeaponType.Ref(), pickinfo.info.parts );
	ZDAMAGETYPE dt = (pickinfo.info.parts==eq_parts_head) ? ZD_BULLET_HEADSHOT : ZD_BULLET;
	...
	else
	{
		...
		pShotInfo->uid = pickinfo.pObject->GetUID();
		...
		pShotInfo->nMeleeType = -1;
	}
	*/
	//Custom: New Antilead: Converting ZAntiLead -> ZShotInfo
    ZShotInfo* pShotInfo = NULL;

    if(ZGetGameTypeManager()->IsQuestDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType()) || pickinfo.pObject->IsNPC() == true || ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_LEAD ) || (pOwner->GetUID().Low != ZGetGameClient()->GetPlayerUID().Low && pickinfo.pObject->GetUID().Low != ZGetGameClient()->GetPlayerUID().Low))
	{
		if(m_pMyCharacter && pickinfo.pObject->GetUID() == m_pMyCharacter->GetUID())
		{
			m_pMyCharacter->OnDamaged(pOwner, pOwner->GetPosition(), dt, pDesc->m_nWeaponType.Ref(), fActualDamage, fRatio );
		}
		else
		{
			pickinfo.pObject->OnDamaged(pOwner, pOwner->GetPosition(), dt, pDesc->m_nWeaponType.Ref(), fActualDamage, fRatio );
	...
	{
		// Custom: NOLEAD
		if( pOwner->GetUID() == ZGetMyUID() && pObject->GetUID() != ZGetMyUID() )
		{
			MUID uidVictim = pObject->GetUID();
			rvector attackerPos = pOwner->GetPosition();
		...
		// TODO: Check this ELSE IF condition. (Disable leading)
		else if( pOwner->GetUID() != ZGetMyUID() && pObject->GetUID() != ZGetMyUID() )
			pObject->OnDamaged(pOwner, pOwner->GetPosition(), dt, pDesc->m_nWeaponType.Ref(), fActualDamage, fRatio );
	}
	*/
	...
	waterSound = GetWorld()->GetWaters()->CheckSpearing( v1, v2, 250, 0.3, !waterSound );
	// Custom: Nolead 'too high ping' check for shotguns. Do not send packets if your ping is too high.
	/*
	if( pObject && pOwner && pOwner->GetUID() == ZGetMyUID() && pObject->GetUID() != ZGetMyUID() )
	{
		MUID uidVictim = pObject->GetUID();
		rvector attackerPos = pOwner->GetPosition();
...
void ZGame::OnPeerAntiLead( const MUID& uidSender, MMatchCharItemParts nSelType, void* pZAntiLeadTemporaryInfoBlob)
{
    int nSize = MGetBlobArrayCount(pZAntiLeadTemporaryInfoBlob);
    ...
    for (int i = 0; i < nSize; ++i)
    {
        char* nPartsType = (char*)MGetBlobArrayElement(pZAntiLeadTemporaryInfoBlob, i);
        if(!nPartsType) break;
...
//			sprintf(szMsg, "%s님이 스스로 패배하였습니다.", szAttacker);
			...
			if (ZGetMyInfo()->IsAdminGrade()) {
				...
				{
					// Custom: Added pAttacker check (stupid MAIET)
					if (pAttacker)
					{
						// Custom: Changed string
						sprintf( szMsg, "^%d%s^9 has committed suicide.",//sprintf( szMsg, "^%d%s^9 스스로 패배",
...
// 		sprintf(szMsg, "%s님이 %s님으로부터 승리하였습니다.", szAttacker, szVictim );
		...
		if (ZGetMyInfo()->IsAdminGrade()) {
			...
			{
				// Custom: Changed string
				sprintf( szMsg, "^%d%s^9 wins,  ^%d%s^9 loses", //sprintf( szMsg, "^%d%s^9 승리,  ^%d%s^9 패배",
...
void ZGame::OnPeerReload(MUID& uid)
{
	...
	if(pCharacter==m_pMyCharacter)
	{
		...
		if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_FASTRELOAD ) && !ZGetGameClient()->GetMatchStageSetting()->IsQuestDrived() )
	...
	case MMATCH_ROUNDSTATE_PLAY:
		{
			...
			//{
				//ZGetScreenEffectManager()->AddScreenEffect("ctf_splash"); // 5 second animation
				//ZGetSoundEngine()->PlaySound("ctf_hint");
			//}
			//else
			if(ZGetConfiguration()->GetEtc()->bAutoRecord && ZGetGame()->m_bRecording) ZGetGame()->ToggleRecording(); //Custom: Stop recording at the end of a round.
			if(ZGetConfiguration()->GetEtc()->bAutoRecord && !ZGetGame()->m_bRecording) ZGetGame()->ToggleRecording(); //Custom: Start recording at the end of a round if not already recording.	
			ZGetScreenEffectManager()->AddRock();			
			if(!ZGetApplication()->m_RunThread) ZGetApplication()->RunAntiThread(); //Custom: Starts anti-shotbot thread at the beginning of rounds.
		}
		break;
	case MMATCH_ROUNDSTATE_FINISH:
		{
			...
			{
				if(ZGetApplication()->m_RunThread) ZGetApplication()->KillAntiThread(); //Custom: Stops anti-shotbot thread at the end of rounds. 
					...
					&& ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_DUELTOURNAMENT)
				{
					...
					if (IsReplay())
					{
						...
						{
							sprintf(szFinishStr, "'%s' dealt %d damage.", pTarget->GetUserName(), pTarget->m_nDamageCaused);
					...
					{
						sprintf(szFinishStr, "You dealt %d damage.", m_pMyCharacter->m_nDamageCaused);
						ZChatOutput(MCOLOR(ZCOLOR_CHAT_SYSTEM), szFinishStr);
					}
					...
					if (m_pMyCharacter && !m_pMyCharacter->IsAdminHide())
					{
						...
						if (vScores.size() > 0)
						{
							...
							sprintf(szFinishStr, "Your team's top scorer: %s (%d damage)", p.first->GetUserName(), p.second );
							ZChatOutput(MCOLOR(ZCOLOR_CHAT_SYSTEM), szFinishStr);
						}
...
void ZGame::OnStageEnterBattle(MCmdEnterBattleParam nParam, MTD_PeerListNode* pPeerNode)
{
	...
	if (uidChar == ZGetMyUID())		// enter한사람이 나자신일 경우
	{
		...
		//Custom: Post modules to server on stage enter battle and it's your character entering.
		ZPostMatchLog();
		//Custom: Auto recording		
		if(ZGetConfiguration()->GetEtc()->bAutoRecord)

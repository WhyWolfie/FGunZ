		...
		if (!m_bSkipUIDrawByRule)
		{
			...
			if( !ZGetGame()->IsReplay() && !m_Observer.IsVisible() && ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DUELTOURNAMENT )
			{
				...
				if( pmyCharacter != NULL )
				{
					...
					//Custom: Personal Damage counter. Consider making ZPostDamageCounter work in non-team environments too!
					if (ZGetGame()->GetMatch()->IsTeamPlay())
					{
						GetClanHPBar(pDC);
						pFont = MFontManager::Get(Z_LOCALE_DEFAULT_FONT);
						pDC->SetFont( pFont );
						if (ZGetGame()->isRecording()) 
							pDC->SetColor(MCOLOR(0, 0, 255));
						else 
							pDC->SetColor(MCOLOR(255, 0, 0));
						sprintf(szMsg, "DMG: %d", pmyCharacter->m_nDamageCaused);

						if( RGetIsWidthScreen() || RGetIsLongScreen() )
							pDC->Text((int)(MGetWorkspaceWidth()-(.09375f*MGetWorkspaceWidth())), (int)25, szMsg);
						else
							pDC->Text((int)(MGetWorkspaceWidth()-75), (int)25, szMsg);
					}
				}
			}
...
void ZCombatInterface::DrawMyNamePont(MDrawContext* pDC)
{
	...
	if ( (ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DUEL) || ( !pCharacter->IsObserverTarget()))
	{
		...
		// Custom: EXP percentage on gauge
		pDC->SetFont(MFontManager::Get("FONTa9b"));
		sprintf_s(buffer, "%d%%", ZGetMyInfo()->GetLevelPercent());
		fCenterVert = 0.134f - (float)pFont->GetHeight()/(float)RGetScreenHeight()/2;
		TextRelative(pDC,170.f/800.f,fCenterVert,buffer);
		pDC->SetFont(pFont);
	}
}
...
void ZCombatInterface::GetClanHPBar(MDrawContext* pDC)
{
	if (!ZGetConfiguration()->GetVideo()->bTeamBar) return;
	float x = 1.f, y = 90.f, hp, ap;
			...
			itor != ZGetGame()->m_CharacterManager.end(); ++itor)
		{
			...
			if (pCharacter->m_nTeamID.Ref() == ZGetGame()->m_pMyCharacter->m_nTeamID.Ref())
			{
				char pUserName[64];
				hp = 0.08 * pCharacter->GetHP() / pCharacter->GetProperty()->fMaxHP.Ref();
				pDC->SetColor(MCOLOR(0x90FF0000));
				pDC->FillRectangle((x / 800.f * 1300), y / 600.f*MGetWorkspaceHeight() + 6, hp * 1100, 4);
				ap = 0.08 * pCharacter->GetAP() / pCharacter->GetProperty()->fMaxAP.Ref();
				pDC->SetColor(MCOLOR(0x9000FF00));
				pDC->FillRectangle((x / 800.f * 1300), y / 600.f*MGetWorkspaceHeight() + 8, ap * 1100, 3);

				MFont *pFont = GetGameFont();
				pFont = MFontManager::Get("FONTa8_O2Wht");
				pDC->SetFont(pFont);
				pDC->SetColor(MCOLOR(0xFFFFFFFF));

				sprintf(pUserName, "%s", pCharacter->GetUserNameA());
				TextRelative(pDC, x / 800.f, (y - 7.f) / 600.f, pUserName);
				y += 14;
				ZPostHPAPInfo(pCharacter->GetHP(),pCharacter->GetAP());
			}
		}
}
...
//#define TEST_CLAN_SCOREBOARD
	...
	// Custom: CQ fixes
	if ( ZGetGameTypeManager()->IsQuestOnly(ZGetGame()->GetMatch()->GetMatchType())
			|| ZGetGameTypeManager()->IsQuestChallengeOnly( ZGetGame()->GetMatch()->GetMatchType()))		// 퀘스트 모드이면...
	{
...
*/
	...
	// NPC 수
	if ( ZGetGameTypeManager()->IsQuestOnly( ZGetGame()->GetMatch()->GetMatchType()) 
		|| ZGetGameTypeManager()->IsQuestChallengeOnly( ZGetGame()->GetMatch()->GetMatchType())) 	// 퀘스트 모드일 경우
	{
...
//		backgroundy=newbackgroundy;
		...
		if ( ZGetGameTypeManager()->IsQuestDerived( ZGetGame()->GetMatch()->GetMatchType()))
		{
			...
			if ( itor != ZGetGame()->m_CharacterManager.end())
			{
				...
				if( pQuestPeerInfo && !pQuestPeerInfo->IsOpened() || !pQuestPeerInfo )
					bDraw = m_Observer.IsVisible(); // Just depend on observer setting.
		...
		// Custom: Set ping colour
		MCOLOR pRealColor = pDC->GetColor();
		
		if( pItem->nPing > 0 && pItem->nPing < 100 )
		{
			//if (!pItem->bDeath)
				pDC->SetColor( MCOLOR(0, 200, 0) );
			//else
			//	pDC->SetColor( MCOLOR(0, 50, 0) );
		}
		else if( pItem->nPing >= 100 && pItem->nPing < 200 )
		{
			//if (!pItem->bDeath)
				pDC->SetColor( MCOLOR(255, 165, 0) );
			//else
			//	pDC->SetColor( MCOLOR(60, 40, 0) );
		}
		else if( pItem->nPing >= 200 && pItem->nPing != MAX_PING )
		{
			//if (!pItem->bDeath)
				pDC->SetColor( MCOLOR(200, 0, 0) );
			//else
			//	pDC->SetColor( MCOLOR(60, 0, 0) );
		}
		else if( pItem->nPing == 999 )
		{
			//if (!pItem->bDeath)
				//pDC->SetColor( MCOLOR(50, 50, 50) );
			//else

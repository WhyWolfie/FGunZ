...
void ZChangeCharParts(RVisualMesh* pVMesh, MMatchSex nSex, int nHair, int nFace, const unsigned long int* pItemID)
{
	...
	if( pItemID[MMCIP_AVATAR] != 0 ) {
		...
		RMeshPartsType mesh_parts_type = eq_parts_etc;
	
		if(pItemID[MMCIP_AVATAR_HEAD] != 0)
		ChangeCharFace(pVMesh, nSex, nFace);
		//Custom: Avatar Clothing Logic - Avatar Override
		for (int i = MMCIP_AVATAR_HEAD; i < MMCIP_END;i++) {
			mesh_parts_type = eq_parts_etc;

			switch (MMatchCharItemParts(i))
			{
				case MMCIP_AVATAR_HEAD:	mesh_parts_type = eq_parts_head;	break;
				case MMCIP_AVATAR_CHEST:	mesh_parts_type = eq_parts_chest;	break;
				case MMCIP_AVATAR_HANDS:	mesh_parts_type = eq_parts_hands;	break;
				case MMCIP_AVATAR_LEGS:	mesh_parts_type = eq_parts_legs;	break;
				case MMCIP_AVATAR_FEET:	mesh_parts_type = eq_parts_feet;	break;
			default: break;
			}
			//This logic sucks, but it basically changes your item if you have an item in that slot.
			if (pItemID[i] != 0 && mesh_parts_type != eq_parts_etc ) {
				MMatchItemDesc* pDesc = MGetMatchItemDescMgr()->GetItemDesc(pItemID[i]);
				if(pDesc)
				{
					pVMesh->SetParts(mesh_parts_type, pDesc->m_pMItemName->Ref().m_szMeshName);
				}
			}
		}
		...
		if (pItemID[MMCIP_HEAD] == 0 && pItemID[MMCIP_AVATAR_HEAD] == 0 ) {
			...
			(ZC_STATE_UPPER_LOAD <= aniState_Upper && aniState_Upper <= ZC_STATE_UPPER_GUARD_CANCEL))
		{
			if(aniState_Upper!=ZC_STATE_UPPER_GUARD_IDLE) {
				...
				if(	(aniState_Lower != ZC_STATE_LOWER_ATTACK1_RET) &&
					...
					// Custom: Sword trail option
				{
					if( ZGetConfiguration()->GetEtc()->bSwordTrail )
						bDrawTracks = true;
					else
						bDrawTracks = false;
				}
...
#define ELSE_IF_SITEM_ENUM(a)	else if(a==m_Items.GetSelectedWeaponType())	{ AddTextEnum(m_Items.GetSelectedWeaponType(),a); }
	...
	//Custom: Avatar Clothing Logic - Avatar Debug State
	ELSE_IF_SITEM_ENUM(MMCIP_AVATAR_HEAD)
	ELSE_IF_SITEM_ENUM(MMCIP_AVATAR_CHEST)
	ELSE_IF_SITEM_ENUM(MMCIP_AVATAR_HANDS)
	ELSE_IF_SITEM_ENUM(MMCIP_AVATAR_LEGS)
	ELSE_IF_SITEM_ENUM(MMCIP_AVATAR_FEET)
...
void ZCharacter::InitProperties()
{
	...
	for (int i = 0; i < MMCIP_END; i++) {
		...
		//Custom: Avatar Clothing Logic - Removal of AP stats from avatar clothing equipped to characters.
		if(i >= MMCIP_AVATAR_HEAD && i <= MMCIP_AVATAR_FEET)
			continue;
	...
	for (int i = 0; i < MMCIP_END; i++) {
	//Custom: Avatar Clothing Logic - Removal of HP stats from avatar clothing equipped to characters.
		if(i >= MMCIP_AVATAR_HEAD && i <= MMCIP_AVATAR_FEET)
			continue;
...
bool ZCharacter::Create(MTD_CharInfo* pCharInfo/*, MTD_CharBuffInfo* pCharBuffInfo*/)
{
	...
	for (int i = 0; i < MMCIP_END; i++) {	///< 아이템 세팅
		//Custom: iLevel/Rarity on equipped gear from net peers.
...
void ZCharacter::InitMeshParts()
{
	...
	if (m_pVMesh)
	{
		...
		for (int i = MMCIP_AVATAR_HEAD; i < MMCIP_END;i++) {
			...
			//This logic sucks, but it basically changes your item if you have an item in that slot.
			if (GetItems() && GetItems()->GetItem(MMatchCharItemParts(i)) && !GetItems()->GetItem(MMatchCharItemParts(i))->IsEmpty() && mesh_parts_type != eq_parts_etc) {
				if(GetItems()->GetItem(MMatchCharItemParts(i))->GetDesc() && GetItems()->GetItem(MMatchCharItemParts(i))->GetDesc()->m_pMItemName)
				{
					m_pVMesh->SetParts(mesh_parts_type, GetItems()->GetItem(MMatchCharItemParts(i))->GetDesc()->m_pMItemName->Ref().m_szMeshName);
				}
			}
...
void ZCharacter::OnDamagedAPlayer(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	if (pAttacker != NULL && fDamage > 0)
	{
		if (pAttacker == ZGetGame()->m_pMyCharacter && this != pAttacker && !ZGetGame()->m_pMyCharacter->IsDie())
		{
			...
			ZPostNoLeadBlob(ZGetGameClient()->GetPlayerUID(), pBlobArray);
...
void ZCharacter::OnDamagedAPlayer(ZObject* pAttacker, vector<MTD_ShotInfo*> vShots)
{
	if (vShots.size() > 0)
	{
		if (ZGetGameClient()->GetPlayerUID() != vShots[0]->uid && GetUID() == vShots[0]->uid)
		{
			if (pAttacker == ZGetGame()->m_pMyCharacter && this != pAttacker && !ZGetGame()->m_pMyCharacter->IsDie())
			{
				...
				for (int i = 0; i < vShots.size(); ++i)
				{
					if (vShots[i] != NULL) delete vShots[i];
				}
			}
		}
	}
...
//Custom: Disabled these for the new antilead, can be re-enabled if you uncomment them in ZCharacter.h to throw people off. 
/*void ZCharacter::OnDamaged_AntiLead(ZObject* pAttacker, rvector srcPos, char nSelType, char nPartsType, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
    if (pAttacker != NULL && fDamage > 0)
    {
        if (pAttacker == ZGetGame()->m_pMyCharacter && this != pAttacker && !ZGetGame()->m_pMyCharacter->IsDie())
        {
            void* pBlobArray = MMakeBlobArray(sizeof(char), 1);
            void* pBlobElement = MGetBlobArrayElement(pBlobArray, 0);

            memcpy(pBlobElement, &nPartsType, sizeof(char));
...
void ZCharacter::OnDamaged_AntiLead(ZObject* pOwner, vector<ZAntiLeadTemporaryInfo*> vInfo, char nSelType)
{
    if (vInfo.size() > 0)
    {
        ...
        {
            int nCount = 0;
            void* pBlobArray = MMakeBlobArray(sizeof(char), vInfo.size());
...
void ZCharacter::OnDamaged_AntiLead(ZObject* pOwner, vector<ZShotInfo*> vInfo)
{
	if (vInfo.size() > 0)
	{
		if (ZGetGameClient()->GetPlayerUID().Low != GetUID().Low && pOwner == ZGetGame()->m_pMyCharacter && pOwner != this && !ZGetGame()->m_pMyCharacter->IsDie())
		{
			int nCount = 0;
			void* pBlobArray = MMakeBlobArray(sizeof(ZShotInfo), vInfo.size());
			for (vector<ZShotInfo*>::iterator itor = vInfo.begin(); itor != vInfo.end(); ++itor)
			{
				ZShotInfo* pInfo = (*itor);
				void* pBlobElement = MGetBlobArrayElement(pBlobArray, nCount);
				memcpy(pBlobElement, pInfo, sizeof(ZShotInfo));

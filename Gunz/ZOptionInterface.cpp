...
void ZOptionInterface::InitInterfaceOption(void)
{
	...
	*/
	...
	//	Button
	{
		...
		pWidget = (MButton*)pResource->FindWidget("WindowedMode");
		if( pWidget )
		{
		...
		pWidget = (MButton*)pResource->FindWidget("TeamHPAPBar");
		if( pWidget )
		{
			pWidget->SetCheck( ZGetConfiguration()->GetVideo()->bTeamBar );
		}

		pWidget = (MButton*)pResource->FindWidget("SwordTrail");
		if( pWidget )
		{
			pWidget->SetCheck( ZGetConfiguration()->GetEtc()->bSwordTrail );
		}
		...
		pWidget = (MButton*)pResource->FindWidget("AutoRecord");
		if( pWidget )
		{
			pWidget->SetCheck( ZGetConfiguration()->GetEtc()->bAutoRecord );
		}
	...
	{	
		...
		if(pComboBox)
		{
			...
			if (IsExist(szCustomFile)) pComboBox->Add("Custom 1"); // Custom: Multiple custom crosshairs

			// Custom: Multiple custom crosshairs
			for (int i = 0; i < 2; ++i)
			{
				char szName[128];
				sprintf_s(szName, "Custom %d", i+2);
				sprintf_s(szCustomFile, "%s%s%d%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, i+2, FN_CROSSHAIR_TAILER);
				if (IsExist(szCustomFile)) pComboBox->Add(szName);
			}
...
//		ZGetInput()->UnregisterActionKey(i);
	...
	*/
	...
	}
	{
		...
		if( pWidget )
		{
			ZGetConfiguration()->GetEtc()->bSwordTrail = pWidget->GetCheck();
		}
		...
		if( pWidget )
		{
			ZGetConfiguration()->GetVideo()->bTeamBar = pWidget->GetCheck();
		}

		pWidget = (MButton*)pResource->FindWidget("AutoRecord");
		if( pWidget )
		{
			ZGetConfiguration()->GetEtc()->bAutoRecord = pWidget->GetCheck();
		}
...
bool ZOptionInterface::IsDiffWindow()
{
	...
	MButton* pButton = (MButton*)pResource->FindWidget("WindowedMode");
	if ( pButton)
		bFullScreen = !pButton->GetCheck();
	else
		return false;

	if ( bFullScreen != Z_VIDEO_FULLSCREEN )
	{
		Z_VIDEO_FULLSCREEN = bFullScreen;
		return true;
	}
...
void ZOptionInterface::ResizeDefaultFont( int newScreenHeight )
{
	...
	//if (RGetIsWidthScreen() || RGetWidthScreen() == 0.8f || RGetWidthScreen() == 0.75f)
	if (!RGetIsLongScreen() && RGetWidthScreen() != 0.8f)
		fontResizeRatio *= 0.95f;
	else if (!RGetIsLongScreen() && RGetWidthScreen() == 0.8f)

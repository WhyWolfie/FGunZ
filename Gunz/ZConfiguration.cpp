...
bool ZConfiguration::LoadConfig(const char* szFileName)
{
	...
	if (!parentElement.IsEmpty())
	{
		...
		if (parentElement.FindChildNode(ZTOK_VIDEO, &childElement))
		{
			...
			childElement.GetChildContents(&m_Video.bTeamBar, ZTOK_VIDEO_TEAMHPBAR ); //Custom: TeamHPAPBar
		...
		if (parentElement.FindChildNode(ZTOK_ETC, &childElement))
		{
			...
			childElement.GetChildContents(&m_Etc.bSwordTrail, ZTOK_ETC_SWORDTRAIL);
...
bool ZConfiguration::SaveToFile(const char *szFileName, const char* szHeader)
{
	...
	// Video
	{
		...
		// TeamHPAPBar
		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_VIDEO_TEAMHPBAR);
		sprintf(temp, "%s", m_Video.bTeamBar ? "true" : "false" );
		aElement.SetContents(temp);
...
//	if( parentElement.FindChildNode(ZTOK_MACRO, &childElement) )
	...
	// Etc
	{
		...
		sprintf(temp, "%s", m_Etc.bSwordTrail?"TRUE":"FALSE");
		aElement.SetContents(temp);
...
void ZConfiguration::Init()
{
	...
	m_Etc.bSwordTrail = true;
	m_Etc.bWeaponRotation = false;
	m_Etc.bAutoRecord = false;
	...
	strcpy( m_Locale.szDefaultFont, "Arial");//"굴림");

...
//Custom: iLevel/Rarity additions, as well as PVE values (if existing)
string GetItemSpecString(MMatchItemDesc* pItemDesc, ZMyItemNode* pNode)
{
	string str;
	char temp[1024];
	...
	else
	{
		...
		if(pItemDesc->m_nDamage.Ref()) {
			...
			if(pNode && pNode->GetItemLevel() != 0)
			{
				int QuestDmg = ((pItemDesc->m_nDamage.Ref() * (pNode->GetItemLevel() + 101)) / 100);

				switch((pNode->GetItemRarity()))
				{
				case 0:
					break;
				case 1:
				...
				sprintf(temp,"%s : %d (PVE: %d) \n", ZMsg( MSG_WORD_ATTACK), pItemDesc->m_nDamage.Ref(), QuestDmg);
				str += temp;
			}
			else
			{
...
void ZShopEquipItem_Match::FillItemDesc(MMatchItemDesc* pItemDesc, MTextArea* pTextArea, ZMyItemNode* pRentalNode)
{
	...
	// 장비 이름
	if(pRentalNode && pItemDesc)
	{

		switch(pRentalNode->GetItemRarity())
		{
		case 0:
			rarity = "^9";
			raritystr = " (Common)";
	...
	pTextArea->SetText( totalName.c_str());
	...
	// 기능
	pTextArea->AddText( "\n");
	pTextArea->AddText( GetItemSpecString(pItemDesc, pRentalNode).c_str(), MCOLOR( 170, 170, 170));
...
void ZShopEquipItem_Match::UpdateCharInfoText()
{
	...
	//Custom: Avatar parts logic.
	if(parts >= MMCIP_AVATAR_HEAD && parts <= MMCIP_AVATAR_FEET)
		return;
...
//Custom: Sacrifice Item in Game Room
ZShopEquipItemHandle_SacMatch::ZShopEquipItemHandle_SacMatch( ZShopEquipItem_Match* pItem )
{
	m_pItem = pItem;
	_ASSERT(m_pItem);
}

bool ZShopEquipItemHandle_SacMatch::GetPrice( int& out_nPrice )
{
	out_nPrice = 0;
...
void ZShopEquipItemHandle_SacMatch::Sell()
{
	...
	else
	{
		if (!m_pItem->GetDesc()->IsCashItem())
		{
			...
			ZMyItemNode* pRentalNode = NULL;
			MUID uidItem = this->GetItemUID();
			pRentalNode = ZGetMyInfo()->GetItemList()->GetItem(uidItem);

			if(!pRentalNode)
				return;

			std::string boxstr = "Are you sure you wish to sacrifice your";
			std::string rarity = "^9";
			...
			boxstr.append(rarity + " ");
			boxstr.append(m_pItem->GetDesc()->m_pMItemName->Ref().m_szItemName);
			boxstr.append(raritystr + " (Item Lv. " + ss.str());
			boxstr.append(") ^9in exchange for an iLevel ");
			boxstr.append(ss.str() + " quest?");
			pDlg->Open(boxstr.c_str(), this);
		}
	}
}

void ZShopEquipItemHandle_SacMatch::OnDone(bool bOk)
{
	if (!bOk) return;
	ZPostRequestSacItem(ZGetGameClient()->GetPlayerUID(), GetItemUID());
}

ZShopEquipItemHandle_SacQuest::ZShopEquipItemHandle_SacQuest( ZShopEquipItem_Quest* pItem )
{
	m_pItem = pItem;
}

bool ZShopEquipItemHandle_SacQuest::GetPrice( int& out_nPrice )
{
	out_nPrice = 0;
	return true;
}

void ZShopEquipItemHandle_SacQuest::Sell()
{
	return;
}

void ZShopEquipItemHandle_SacQuest::OnDone(int nCount)
{
	return;
}


ZShopEquipItemHandle_SacGamble::ZShopEquipItemHandle_SacGamble( ZShopEquipItem_Gamble* pItem )
{
	m_pItem = pItem;
}

bool ZShopEquipItemHandle_SacGamble::GetPrice( int& out_nPrice )
{
	out_nPrice = 0;
	return true;
}

void ZShopEquipItemHandle_SacGamble::Sell()
{
	return;
}

void ZShopEquipItemHandle_SacGamble::OnDone(int nCount)
{
	return;
}

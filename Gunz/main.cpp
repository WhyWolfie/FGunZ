...
void CrcFailExitApp() { 
	// Custom: (Any CRC checksum of data values that fail will enter this function)
#ifdef _PUBLISH
...
RRESULT OnCreate(void *pParam)
{
	...
	sprintf( cstrReleaseDate, "Coded by Linear88, Mike, and SecretsOTheP" );
...
{
#ifdef _PUBLISH
	g_ModeParams.bFullScreen = ZGetConfiguration()->GetVideo()->bFullScreen; //true;
#else
	#ifdef _DEBUG
		g_ModeParams.bFullScreen = ZGetConfiguration()->GetVideo()->bFullScreen; //false;

		if (g_ModeParams.bFullScreen)
			MessageBox( g_hWnd, "Launching in fullscreen!", "Gunz", MB_ICONWARNING | MB_OK );
	#else
...
//		AfxMessageBox(_T("_AhnHS_SaveFuncAddress Failed!"));
	...
	if (bCompatibilityMode)
	{
		...
		MessageBox(g_hWnd, "Failed to run Freestyle GunZ in compatibility mode.\nPlease remove compatibility mode settings and launch the game again!", "Freestyle GunZ", MB_OK | MB_ICONERROR);
		return false;
	}

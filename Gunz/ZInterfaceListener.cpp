...
public:
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage){
		if(MWidget::IsMsg(szMessage, MBTN_CLK_MSG)==true){
			...
			if(nPlayerCnt > pStageSetting->nMaxPlayers && nPlayerCnt!=0 && pStageSetting->nMaxPlayers!=0 && (ZGetGameTypeManager()->IsQuestDerived(pStageSetting->nGameType) || pStageSetting->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE))
			{
				// 최대인원보다 방인원이 많을시 처리
				char szText[128] = {0,};
...
BEGIN_IMPLEMENT_LISTENER(ZGetSacEquipButtonListener, MBTN_CLK_MSG)
	ZGetGameInterface()->GetShopEquipInterface()->OnEquipSacButton();
END_IMPLEMENT_LISTENER()

	
BEGIN_IMPLEMENT_LISTENER(ZGetSacShopButtonListener, MBTN_CLK_MSG)
	ZGetGameInterface()->GetShopEquipInterface()->OnShopSacButton();
END_IMPLEMENT_LISTENER()
...
BEGIN_IMPLEMENT_LISTENER( ZGetShopEquipmentSwitchViewListener, MBTN_CLK_MSG)
	ZGetGameInterface()->SwitchShopEquipmentView();
END_IMPLEMENT_LISTENER()

...
void ZMyItemList::SerializeZItemList()
{
	...
	ZShopEquipItemHandle_SacMatch* pHandleSac;
	...
	for (int i = 0; i < (int)m_ItemIndexVector.size(); i++)
	{
		...
		if (itor != m_ItemMap.end())
		{
			...
			pHandleSac = new ZShopEquipItemHandle_SacMatch(pWrappedItem);
			pHandleSell->SetItemUID(uidItem);
			pHandleSac->SetItemUID(uidItem);
			pWrappedItem->SetHandleSell(pHandleSell);
			pWrappedItem->SetHandleSac(pHandleSac);
...
void ZMyItemList::SerializeQItemList()
{
	...
	ZShopEquipItemHandle_SacQuest* pHandleSac;
	...
	for ( MQUESTITEMNODEMAP::iterator itor = m_QuestItemMap.begin();  itor != m_QuestItemMap.end();  itor++)
	{
		...
		pHandleSac = new ZShopEquipItemHandle_SacQuest(pWrappedItem);
...
void ZMyItemList::SerializeGItemList()
{
	...
	ZShopEquipItemHandle_SacGamble* pHandleSac;
	...
	for( DWORD i = 0;i < dwSize; ++i )
	{
		...
		pHandleSac = new ZShopEquipItemHandle_SacGamble(pWrappedItem);
...
void ZMyItemList::SetItemsAll(MTD_ItemNode* pItemNodes, const int nItemCount)
{
	...
	for (int i = 0; i < nItemCount; i++) {
		...
		if (bIsRentItem  == false) {		///< 일반 아이템			
			pNewItemNode->Create(pItemNodes[i].uidItem, pItemNodes[i].nItemID, pItemNodes[i].nCount, false, RENT_MINUTE_PERIOD_UNLIMITED, 0, pItemNodes[i].nRarity, pItemNodes[i].niLevel);
		} else {							///< 기간제 아이템
			pNewItemNode->Create(pItemNodes[i].uidItem, pItemNodes[i].nItemID, pItemNodes[i].nCount, bIsRentItem, pItemNodes[i].nRentMinutePeriodRemainder, pItemNodes[i].iMaxUseHour, pItemNodes[i].nRarity, pItemNodes[i].niLevel);
		}
...
int ZMyItemList::GetEquipedTotalWeight()
{
	...
	for (int i=0; i < MMCIP_END; i++)
	{
		//Custom: Avatar Clothing - Removal of total weight allowed increases from avatar clothing.
...
int ZMyItemList::GetEquipedHPModifier()
{
	...
	for (int i=0; i < MMCIP_END; i++)
	{
		...
		//Custom: Avatar Clothing - Removal of HP increases from avatar clothing.
		if(i >= MMCIP_AVATAR_HEAD && i <= MMCIP_AVATAR_FEET)
			continue;
...
int ZMyItemList::GetEquipedAPModifier()
{
	...
	for (int i=0; i < MMCIP_END; i++)
	{
		...
		//Custom: Avatar Clothing - Removal of AP allowed increases from avatar clothing.
		if(i >= MMCIP_AVATAR_HEAD && i <= MMCIP_AVATAR_FEET)
			continue;
...
int ZMyItemList::GetMaxWeight()
{
	...
	for (int i=0; i < MMCIP_END; i++)
	{
		...
		//Custom: Avatar Clothing - Removal of weight increases from avatar clothing.
		if(i >= MMCIP_AVATAR_HEAD && i <= MMCIP_AVATAR_FEET)
			continue;

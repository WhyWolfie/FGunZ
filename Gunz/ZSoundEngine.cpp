...
#include "dirent.h" // Custom: MP3 Player
...
// Custom: MP3 Player
#define SOUNDCUSTOM_DIR "custom/music/"
vector<string> g_vMusic;
int g_nMusicNextIndex = 0;
...
bool ZSoundEngine::Create(HWND hwnd, bool bHWMixing, ZLoadingProgress *pLoadingProgress )
{
	...
	// Custom: MP3 player
	DIR* pDir = opendir( "CUSTOM/music" );

	if( !pDir )
	{
		mlog("fail to find custom music\n");
		return true;
	}

	while( dirent* pDInfo = readdir( pDir ) )
	{
		if( !stricmp( &pDInfo->d_name[strlen( pDInfo->d_name ) - 4], ".wav" ) ||
			!stricmp( &pDInfo->d_name[strlen( pDInfo->d_name ) - 4], ".raw" ) ||
			!stricmp( &pDInfo->d_name[strlen( pDInfo->d_name ) - 4], ".mp2" ) ||
			!stricmp( &pDInfo->d_name[strlen( pDInfo->d_name ) - 4], ".mp3" ) ||
			!stricmp( &pDInfo->d_name[strlen( pDInfo->d_name ) - 4], ".ogg" )
			)
		{
			string strAdd = "";
			if( !strstr( pDInfo->d_name, SOUNDCUSTOM_DIR ) )
				strAdd = SOUNDCUSTOM_DIR;

			strAdd.append( pDInfo->d_name );

			g_vMusic.push_back( strAdd );
		}
	}

	closedir( pDir );

	mlog("found %d custom music\n", g_vMusic.size());

	// Shuffle now. I love STL
	random_shuffle( g_vMusic.begin(), g_vMusic.end() );
...
bool ZSoundEngine::OpenMusic(const char* szFileName, MZFileSystem* pfs)
{
	...
	if( g_vMusic.size() > 0 )
	{
		...
		{
			if( !stricmp( g_vMusic[i].c_str(), szFileName ) )
			{
				// Native filesystem FTW
#ifdef _PUBLISH
...
void ZSoundEngine::MusicEndCallback(void* pCallbackContext)
{
	...
	// Custom: MP3 Player
	if (pSoundEngine->m_bBattleMusic || g_vMusic.size() > 0)
	{
		// Custom: Stop music
		if( g_vMusic.size() > 0 )
		{
...
#define BGM_FOLDER		"Sound/BGM/"
	...
	if( g_vMusic.size() > 0 )
	{
		...
		{
			++g_nMusicNextIndex; // Music already shuffled, so just play in order

			if( g_nMusicNextIndex >= g_vMusic.size() )
				g_nMusicNextIndex = 0;
		}

		if( strstr( g_vMusic[g_nMusicNextIndex].c_str(), SOUNDCUSTOM_DIR ) )
			sprintf( szFileName, "%s", g_vMusic[g_nMusicNextIndex].c_str() );
		else
			sprintf( szFileName, "%s%s", SOUNDCUSTOM_DIR, g_vMusic[g_nMusicNextIndex].c_str() );

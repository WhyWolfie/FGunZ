...
//Custom: iLevel/Rarity on equipped gear.
bool ZCharacterItem::EquipItem(MMatchCharItemParts parts, int nItemDescID, int nItemCount, int nRarity, int niLevel)
{
	if (nItemDescID == 0) {
	...
	// Custom: Disabled ASSERT on pDesc == NULL
...
//		_ASSERT(0);
	...
	//Custom: iLevel/Rarity on equipped gear.
	m_Items[parts].Create(MUID(0,0), pDesc, 1, nItemCount, nRarity, niLevel);
	return true;
}

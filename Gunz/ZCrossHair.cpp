...
void ZCrossHair::GetBitmaps(MBitmap** ppoutBitmap, MBitmap** ppoutPickBitmap, ZCrossHairPreset nPreset)
{
	...
	// Custom: Multiple custom crosshairs
	if (nPreset < ZCSP_CUSTOM1)
	...
	else
	{
		...
		{
			int nPresetNumber = (nPreset == ZCSP_CUSTOM2) ? 2 : 3;

			sprintf(szBar[CH_CENTER],	"%s%s%d%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_TAILER);
			sprintf(szBar[CH_TOP],		"%s%s%d%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_TOP, FN_CROSSHAIR_TAILER);
			sprintf(szBar[CH_BOTTOM],	"%s%s%d%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_BOTTOM, FN_CROSSHAIR_TAILER);
			sprintf(szBar[CH_LEFT],		"%s%s%d%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_LEFT, FN_CROSSHAIR_TAILER);
			sprintf(szBar[CH_RIGHT],	"%s%s%d%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_RIGHT, FN_CROSSHAIR_TAILER);

			sprintf(szPick[CH_CENTER],	"%s%s%d%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_PICK, FN_CROSSHAIR_TAILER);
			sprintf(szPick[CH_TOP],		"%s%s%d%s%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_TOP, FN_CROSSHAIR_PICK, FN_CROSSHAIR_TAILER);
			sprintf(szPick[CH_BOTTOM],	"%s%s%d%s%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_BOTTOM, FN_CROSSHAIR_PICK, FN_CROSSHAIR_TAILER);
			sprintf(szPick[CH_LEFT],	"%s%s%d%s%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_LEFT,	FN_CROSSHAIR_PICK, FN_CROSSHAIR_TAILER);
			sprintf(szPick[CH_RIGHT],	"%s%s%d%s%s%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, nPresetNumber, FN_CROSSHAIR_RIGHT,	FN_CROSSHAIR_PICK, FN_CROSSHAIR_TAILER);
		}
	}
...
void ZCrossHair::ChangeFromOption()
{
	...
	if (pComboBox)
	{
		...
		// Custom: Multiple custom crosshairs
		if ((nSelIndex >= 0) && (nSelIndex <= ZCSP_CUSTOM3))
		{

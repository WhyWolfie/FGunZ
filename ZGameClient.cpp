									  ...
									  const MUID& uidPlayer, bool bEnabledSurvivalMode, bool bEnabledDuelTournament, unsigned char* pbyGuidReqMsg)
{
	...
	m_LobbyChat = (nUGradeID >= MMUG_EVENTTEAM && nUGradeID != MMUG_BLOCKED); //Custom: Auto-enable Lobbychat for staff.
	m_bAdminNAT = (nUGradeID >= MMUG_EVENTTEAM && nUGradeID != MMUG_BLOCKED); // Custom: Admin NAT
...
//	gid = MMUG_DEVELOPER;
	...
	{
		wsprintf(szText, "^2[Lobby]%s: %s", szName, szChat); //This will have to be formatted to FGunZ's styleff
		ZChatOutput(szText);
	}
}
...
//#define RMOD2(x, y, type) for( int i = 0; i < vTagArray.size(); ++i ){ if( !strstr( vTagArray[i].c_str(), x ) ) continue; int nVal = -1;string strSearch = string(x) + string("%d") + string(y);sscanf( vTagArray[i].c_str(), strSearch.c_str(), &nVal );if( nVal < INT_MIN || nVal > INT_MAX )nVal = -1; if( nVal >= 0 )m_MatchStageSetting.InsertRoomModifier( type, nVal );}
				...
				RMOD( "[ks]", MMOD_NOKSTYLE )
				RMOD( "[wf]", MMOD_WIREFRAME )
				RMOD( "[esp]", MMOD_ESP )
...
#undef RMOD_BEGIN
	...
	if (uidChar == GetPlayerUID())
	{
		...
		if(!ZGetGameClient()->GetMatchStageSetting()->IsUserExempt(ZGetMyInfo()->GetUGradeID())) // Custom: Staff Exception Check
		{
			...
			if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_NG ) )
			{
				ZPostRequestTakeoffItem(ZGetGameClient()->GetPlayerUID(), MMCIP_PRIMARY);
				ZPostRequestTakeoffItem(ZGetGameClient()->GetPlayerUID(), MMCIP_SECONDARY);
			}

			if( ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_NI ) )
			{
				ZPostRequestTakeoffItem(ZGetGameClient()->GetPlayerUID(), MMCIP_CUSTOM1);
				ZPostRequestTakeoffItem(ZGetGameClient()->GetPlayerUID(), MMCIP_CUSTOM2);
			}
...
//			unsigned long nChecksum = MGetMatchItemDescMgr()->GetChecksum();
			...
			char szGunz64EncryptMD5Value[MAX_MD5LENGH] = {0, };
			char szSystemEncryptMD5Value[MAX_MD5LENGH] = {0, };
			char szAHEncryptMD5Value[MAX_MD5LENGH] = {0, };
			GetEncryptMD5HashValue(szEncryptMD5Value, "Gunz.exe");
			GetEncryptMD5HashValue(szGunz64EncryptMD5Value, "Gunz64.exe");
			GetEncryptMD5HashValue(szSystemEncryptMD5Value, "system.fgz");
			memset( szAHEncryptMD5Value, 0, MAX_MD5LENGH );

			char szSecretString[ MAX_MD5LENGH ] = {0, };
			// 15 chars + null terminator = 16
			strncpy_s( szSecretString, "2Cf715a1sf0978d", MAX_MD5LENGH );
			m_ServerPacketCrypter.Encrypt( szSecretString, MAX_MD5LENGH, (MPacketCrypterKey *)m_ServerPacketCrypter.GetKey() );

#ifdef LOCALE_NHNUSA
			// Custom: Default Login
			// Custom: Replace this login when building for offical release
			//ZPostLogin(szID, szPassword, nChecksum, ("0c04fbf66a2beb4dc4131be040de4a86"), ("a91376af95212d40ae9a461e0b23e934"),("68aecd8af47942380b5cd21f3debbe99"), "25FEAF93FF00-5C");
			ZPostLogin(szID, szPassword, nChecksum, szEncryptMD5Value, szGunz64EncryptMD5Value, szSystemEncryptMD5Value, "MIKEMIKEMIKE-5C", szSecretString, szAHEncryptMD5Value);
			// IJJI
...
void ZGameClient::GetEncryptMD5HashValue(char* szEncryptMD5Value, char* szModule)							// update sgk 0724
{
	...
	strcpy_s( filePath, string( filePath ).substr( 0, pos ).append("\\GunzReal.exe").c_str() );


	// Just overwrite the path
	if (!stricmp(szModule, "Gunz64.exe"))
	{
		string::size_type pos = string( filePath ).find_last_of( "\\/" );
		strcpy_s( filePath, string( filePath ).substr( 0, pos ).append("\\Gunz64.exe").c_str() );
	}
	else if (!stricmp(szModule, "system.fgz"))
	{
		string::size_type pos = string( filePath ).find_last_of( "\\/" );
		strcpy_s( filePath, string( filePath ).substr( 0, pos ).append("\\system.fgz").c_str() );
	}
	else if (!stricmp(szModule, "fg.dll"))
	{
		string::size_type pos = string( filePath ).find_last_of( "\\/" );
		strcpy_s( filePath, string( filePath ).substr( 0, pos ).append("\\fg.dll").c_str() );
	}

#ifdef _DEBUG
	// paths for real hashes can be put here
	if (!stricmp(szModule, "Gunz.exe"))
	{
		if (pMD5->md5_file("\\Gunz.exe", szMD5Value) != 0)
			return;
	}
	else if (!stricmp(szModule, "Gunz64.exe"))
	{
		if (pMD5->md5_file("\\Gunz64.exe", szMD5Value) != 0)
			return;
	}
	else if (!stricmp(szModule, "system.fgz"))
	{
		if (pMD5->md5_file("\\system.fgz", szMD5Value) != 0)
			return;
	}
#else

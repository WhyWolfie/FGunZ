...
void ZShopEquipListbox::SetupItemDescTooltip()
{
	...
	if (pItemDescTextArea)
	{
		...
		/*
		{
			ZShopEquipListItem* pItem = (ZShopEquipListItem*)GetSelItem();
			MRECT rcListBox = GetRect();
			MRECT rcItem;

			if (pItem)
			{
				pItem->GetIconRect(rcItem, rcItem);
...
public:
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage)
	{
		if(MWidget::IsMsg(szMessage, MLB_ITEM_SEL) == true)
		{
			...
			// Custom: Set tooltip
			//pEquipmentListBox->SetupItemDescTooltip();
...
public:
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage)
	{
		if ( MWidget::IsMsg(szMessage, MLB_ITEM_SEL)==true) {
			...
			//Custom: Quest sacrifice enable conditions
			if(ZGetGameClient() && ZGetGameClient()->GetMatchStageSetting() && ZGetGameClient()->GetMatchStageSetting()->GetStageSetting())
			{
				...
				if(pStageSetting->nGameType == MMATCH_GAMETYPE_QUEST)
				{
					...
					{
					WidgetEnableShow("SacEquipConfirmCaller", pListItem->GetItemData()->CanSell(), true);
					}
				}
			}
		...
		if(MWidget::IsMsg(szMessage, MLB_ITEM_SEL)==true)
		{
			...
			if(ZGetGameClient() && ZGetGameClient()->GetMatchStageSetting() && ZGetGameClient()->GetMatchStageSetting()->GetStageSetting())
			{
				...
				if(pStageSetting->nGameType == MMATCH_GAMETYPE_QUEST)
				{
					if (ZGetGameClient()->AmIStageMaster())
					{
					WidgetEnableShow("SacShopConfirmCaller", pListItem->GetItemData()->CanSell(), true);
					}
				}
			}
			return true;

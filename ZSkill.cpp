...
{
	// Custom: Don't read zskill cache for debug
	/*
#ifdef _DEBUG
	...
	*/
	...
	// Custom: Don't write zskill cache for debug
	/*
#ifdef _DEBUG
		...
		for(ZObjectManager::iterator i = ZGetObjectManager()->begin();i!=ZGetObjectManager()->end();i++) {
			...
			if(CheckRange(pTarget->GetPosition(),pTarget) && m_pDesc->CheckResist(pTarget,&fDamage)) {
				if(ZGetGame()->CanAttack(m_pOwner,pTarget)) {
					if(fDamage)
					{
						if(ZGetGame()->m_pMyCharacter && pTarget->GetUID() == ZGetGame()->m_pMyCharacter->GetUID())
						{
							ZGetGame()->m_pMyCharacter->OnDamaged(m_pOwner,m_TargetPos,ZD_MELEE,MWT_KATANA,fDamage,m_pDesc->nModCriticalRate*.01f);
						}
						else
						{
		...
		for ( ZObjectManager::iterator i = ZGetObjectManager()->begin();  i!=ZGetObjectManager()->end();  i++)
		{
			...
			if ( CheckRange( pTargetObject->GetPosition(), pObject) && m_pDesc->CheckResist( pObject, &fDamage))
			{
				...
				if ( ZGetGame()->CanAttack( m_pOwner, pObject))
				{
					...
					{
						if(ZGetGame()->m_pMyCharacter && pObject->GetUID() == ZGetGame()->m_pMyCharacter->GetUID())
						{
							ZGetGame()->m_pMyCharacter->OnDamaged(m_pOwner,m_TargetPos,ZD_MELEE,MWT_KATANA,fDamage,m_pDesc->nModCriticalRate*.01f);
						}
						else
